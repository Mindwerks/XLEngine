...
void DaggerXL_Player::Update(uint32_t uObjID, uint32_t uParamCount, LogicParam *param) {
    ...
    if (0)
    {
        ...
        if ((fMoveSpeed > fDesSpeed && s > 0.0f) || (fMoveSpeed < fDesSpeed && s < 0.0f))
        { fMoveSpeed = fDesSpeed; }
    }
    ...
    if (vel.z == 0 && !bLevitating)
    {
        if (m_bOnGround)
        {
            ...
            m_pAPI->World_Collide(&vLoc, &vGravLoc, m_PhysicsData->m_uSector, fPlayerRadius,
                                  m_bPassthruAdjoins ? 1 : 0);
