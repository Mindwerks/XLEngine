...
int LFD_Film::ParseCommand(int32_t cmd, char *pData) {
    ...
    switch (cmd)
    {
        ...
        case CMD_CUT:
        {
            int16_t cutHow = *((int16_t *) &pData[index]);
            index += 2;
            int16_t cutType = *((int16_t *) &pData[index]);
            index += 2;
            int16_t unknown = *((int16_t *) &pData[index]);
            index += 2;
            printf("cut=%d, %d", cutHow, cutType);

            Command *pCmd = &m_CommandPool[m_nCmdPoolIdx];
            m_nCmdPoolIdx++;
            pCmd->pData = pFile;
            pCmd->type = m_nEntry;
            pCmd->cmd = CMD_CUT;
        ...
        case CMD_STEREO:
        {
            int16_t onOff = *((int16_t *) &pData[index]);
            index += 2;
            int16_t vol = *((int16_t *) &pData[index]);
            index += 2;
            int16_t vA = *((int16_t *) &pData[index]);
            index += 2;
            int16_t vB = *((int16_t *) &pData[index]);
            index += 2;
            int16_t pan = *((int16_t *) &pData[index]);
            index += 2;
            Command *pCmd = &m_CommandPool[m_nCmdPoolIdx];
            m_nCmdPoolIdx++;
            pCmd->pData = pFile;
        ...
        case CMD_LAYER:
        {
            int16_t layer = *((int16_t *) &pData[index]);
            index += 2;
        ...
        case CMD_ANIMATE:
        {
            int16_t dir = *((int16_t *) &pData[index]);
            index += 2;
            int16_t move = *((int16_t *) &pData[index]);
            index += 2;
        ...
        case CMD_FRAME:
        {
            int16_t frame = *((int16_t *) &pData[index]);
            index += 2;
            int16_t move = *((int16_t *) &pData[index]);
            index += 2;
        ...
        case CMD_CUE:
        {
            int16_t cue = *((int16_t *) &pData[index]);
            index += 2;
        ...
        case CMD_MOVE:
        {
            int16_t x = *((int16_t *) &pData[index]);
            index += 2;
            int16_t y = *((int16_t *) &pData[index]);
            index += 2;
        ...
        case CMD_SPEED:
        {
            int16_t r = *((int16_t *) &pData[index]);
            index += 2;
            int16_t d = *((int16_t *) &pData[index]);
            index += 2;
...
bool LFD_Film::Start(Archive *pRes0, Archive *pRes1, const char *pszFile, uint32_t uFlags, int32_t nSpeed) {
    ...
    char szName[32];
    char szFile[32];
    m_nFilmLength = *((int16_t *) &pData[index]);
    index += 2;
    nNumEntries = *((int16_t *) &pData[index]);
    index += 2;
    ...
    {
        pEntry = (FilmEntry *) &pData[index];
        index += sizeof(FilmEntry);
        //commands
        switch (pEntry->nBlockType)
        {
            ...
            case BTYPE_DELT_ANIM_CUST:
                ...
                if (pEntry->szType[0] == 'D' && pEntry->szType[1] == 'E' && pEntry->szType[2] == 'L' &&
                    pEntry->szType[3] == 'T')
                {
                    memcpy(szName, pEntry->szName, 8);
                    szName[8] = 0;
                ...
                else if (pEntry->szType[0] == 'A' && pEntry->szType[1] == 'N' && pEntry->szType[2] == 'I' &&
                         pEntry->szType[3] == 'M')
                {
                    memcpy(szName, pEntry->szName, 8);
                    szName[8] = 0;
                    ...
                    if (stricmp(pGraphic->szFile, "moceyes") == 0 || stricmp(pGraphic->szFile, "montalk1") == 0 ||
                        stricmp(pGraphic->szFile, "montalk2") == 0 ||
                ...
                else if (pEntry->szType[0] == 'C' && pEntry->szType[1] == 'U' && pEntry->szType[2] == 'S' &&
                         pEntry->szType[3] == 'T')
                {
                    m_nEntry = BTYPE_CUST;
                }
        ...
        {
            size = *((int16_t *) &pData[index]);
            index += 2;
            cmd = *((int16_t *) &pData[index]);
            index += 2;
    ...
    for (i = 201; i >= 0; i--)
    {
        if (m_apAnimQueue[i])
        {
            ...
            while (pCur)
            {
                ...
                pCur->pAnim->Render(pCur->nFrame, (float) pCur->nX * fOO320, ((float) pCur->nY * fOO200), 1000.0f,
                                    -1000.0f, 0.0f, 0.0f, bDistort);
                pCur = pCur->pNext;
            };
        }
    }
