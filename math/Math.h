...
public:
    ...
    static bool
    LineIntersect2D(const Vector2 &p0, const Vector2 &p1, const Vector2 &p2, const Vector2 &p3, float &s, float &t) {
    ...
    static bool
    EdgeIntersectsBox2D(const Vector2 &vAABB_Min, const Vector2 &vAABB_Max, const Vector2 &v0, const Vector2 &v1) {
    ...
    static float BiCubic_Heightfield(float p[4][4], float x, float y) {
        ...
        float a11 = p[0][0] / 9 + p[0][1] / 6 - p[0][2] / 3 + p[0][3] / 18 + p[1][0] / 6 + p[1][1] / 4 - p[1][2] / 2 +
                    p[1][3] / 12 - p[2][0] / 3 - p[2][1] / 2 + p[2][2] - p[2][3] / 6 + p[3][0] / 18 + p[3][1] / 12 -
                    p[3][2] / 6 + p[3][3] / 36;
        float a12 = p[0][1] / 3 - p[0][0] / 6 - p[0][2] / 6 - p[1][0] / 4 + p[1][1] / 2 - p[1][2] / 4 + p[2][0] / 2 -
                    p[2][1] + p[2][2] / 2 - p[3][0] / 12 + p[3][1] / 6 - p[3][2] / 12;
        float a13 = p[0][0] / 18 - p[0][1] / 6 + p[0][2] / 6 - p[0][3] / 18 + p[1][0] / 12 - p[1][1] / 4 + p[1][2] / 4 -
                    p[1][3] / 12 - p[2][0] / 6 + p[2][1] / 2 - p[2][2] / 2 + p[2][3] / 6 + p[3][0] / 36 - p[3][1] / 12 +
                    p[3][2] / 12 - p[3][3] / 36;
        float a20 = p[0][1] / 2 - p[1][1] + p[2][1] / 2;
        float a21 = p[0][2] / 2 - p[0][1] / 4 - p[0][0] / 6 - p[0][3] / 12 + p[1][0] / 3 + p[1][1] / 2 - p[1][2] +
                    p[1][3] / 6 - p[2][0] / 6 - p[2][1] / 4 + p[2][2] / 2 - p[2][3] / 12;
        float a22 = p[0][0] / 4 - p[0][1] / 2 + p[0][2] / 4 - p[1][0] / 2 + p[1][1] - p[1][2] / 2 + p[2][0] / 4 -
                    p[2][1] / 2 + p[2][2] / 4;
        float a23 = p[0][1] / 4 - p[0][0] / 12 - p[0][2] / 4 + p[0][3] / 12 + p[1][0] / 6 - p[1][1] / 2 + p[1][2] / 2 -
                    p[1][3] / 6 - p[2][0] / 12 + p[2][1] / 4 - p[2][2] / 4 + p[2][3] / 12;
        float a30 = p[1][1] / 2 - p[0][1] / 6 - p[2][1] / 2 + p[3][1] / 6;
        float a31 = p[0][0] / 18 + p[0][1] / 12 - p[0][2] / 6 + p[0][3] / 36 - p[1][0] / 6 - p[1][1] / 4 + p[1][2] / 2 -
                    p[1][3] / 12 + p[2][0] / 6 + p[2][1] / 4 - p[2][2] / 2 + p[2][3] / 12 - p[3][0] / 18 -
                    p[3][1] / 12 + p[3][2] / 6 - p[3][3] / 36;
        float a32 = p[0][1] / 6 - p[0][0] / 12 - p[0][2] / 12 + p[1][0] / 4 - p[1][1] / 2 + p[1][2] / 4 - p[2][0] / 4 +
                    p[2][1] / 2 - p[2][2] / 4 + p[3][0] / 12 - p[3][1] / 6 + p[3][2] / 12;
        float a33 =
                p[0][0] / 36 - p[0][1] / 12 + p[0][2] / 12 - p[0][3] / 36 - p[1][0] / 12 + p[1][1] / 4 - p[1][2] / 4 +
                p[1][3] / 12 + p[2][0] / 12 - p[2][1] / 4 + p[2][2] / 4 - p[2][3] / 12 - p[3][0] / 36 + p[3][1] / 12 -
                p[3][2] / 12 + p[3][3] / 36;
