...
MAX_LOADSTRING);
LoadString(hInstance, IDC_XLENGINE, szWindowClass,
MAX_LOADSTRING);
...
sprintf(szSettingsFile,
"%s/%s.conf", szGame, szGame);
EngineSettings::SetGameDir(szGame);
EngineSettings::Load( szSettingsFile );
EngineSettings::SetStartMap( szMap );
EngineSettings::SetMultiplayerData( nServer_PlayerCnt, nPort, nServer_PlayerCnt
> 0 ? szServerIP : szJoinIP );

// Perform application initialization:
if ( !
InitInstance(hInstance, nCmdShow, EngineSettings::GetScreenWidth(), EngineSettings::GetScreenHeight()
))
{
return
FALSE;
...
(g_bHasFocus || g_bFullScreen))
{
//Get the current mouse cursor position.
GetCursorPos(&point);
//compute the center point of the window.
center.
x = EngineSettings::GetScreenWidth() >> 1;
center.
y = EngineSettings::GetScreenHeight() >> 1;
...
hInstance = hInstance;
wcex.
hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_XLENGINE));
wcex.
...
g_bFullScreen = false;
if (
EngineSettings::IsFeatureEnabled(EngineSettings::FULLSCREEN)
&& !
...
0,sizeof(dmScreenSettings));    // Makes Sure Memory's Cleared
dmScreenSettings.
dmSize = sizeof(dmScreenSettings);        // Size Of The Devmode Structure
dmScreenSettings.
dmPelsWidth = w;                    // Selected Screen Width
dmScreenSettings.
dmPelsHeight = h;                    // Selected Screen Height
dmScreenSettings.
...
hWnd = CreateWindow(szWindowClass, szTitle, dwStyle, CW_USEDEFAULT, 0, desiredSize.right - desiredSize.left,
                    desiredSize.bottom - desiredSize.top, NULL, NULL, hInstance, NULL);
}
else
{
...
case WM_XBUTTONDOWN:
if ( g_bHasFocus || g_bFullScreen )
Input::SetKeyDown( HIWORD(wParam)
==MK_XBUTTON1 ? VK_XBUTTON1 : VK_XBUTTON2 );
break;
case WM_LBUTTONUP:
if ( g_bHasFocus || g_bFullScreen )
