...
int32_t Parser::SearchKeyword_int32_t(const char *pszKeyword, int32_t &rnValue) {
    ...
    for (uint32_t c = m_uFilePtr; c < m_uLength; c++)
    {
        ...
        if (bFound)
        {
            ...
            int i = 0, cc;
            for (cc = c + (int) strlen(pszKeyword); m_pMemory[cc] != '\r' && m_pMemory[cc] != '#' &&
                                                    ((m_pMemory[cc] != ' ' && m_pMemory[cc] != 9) ||
                                                     !bStartSpace); cc++, i++)
            {
                szValue[i] = m_pMemory[cc];
                if (szValue[i] != ' ' && szValue[i] != 9)
                { bStartSpace = true; }
            }
            szValue[i] = 0;
            m_uFilePtr = cc;
...
bool Parser::SearchKeyword_Str(const char *pszKeyword, char *pszOut, bool bEndOnSpace/*=true*/,
                               bool bPreserveSpaces/*=false*/, int32_t maxSpaceCnt/*=4*/) {
    bool bRet = false;
    char szValue[256] = "";
    pszOut[0] = 0;
    bool bStartSpace = false;
    ...
    for (c = (int32_t) uStart; c < (int32_t) uEnd; c++)
    {
        ...
        {
            //now search for a number or end of the line.
            for (cc = c + (int32_t) strlen(pszKeyword), i = 0; m_pMemory[cc] != '\r' && m_pMemory[cc] != '#' &&
                                                               ((m_pMemory[cc] != ' ' && m_pMemory[cc] != 9) ||
                                                                !bStartSpace ||
                                                                (!bEndOnSpace && nSpaceCnt < maxSpaceCnt)); cc++)
            {
...
bool Parser::SearchKeyword_F1(const char *pszKeyword, float &rfValue) {
    ...
    for (c = (int32_t) m_uFilePtr; c < (int32_t) m_uLength; c++)
    {
        ...
        {
            //now search for a number or end of the line.
            for (cc = c + (int) strlen(pszKeyword), i = 0; m_pMemory[cc] != '\r' && m_pMemory[cc] != '#' &&
                                                           ((m_pMemory[cc] != ' ' && m_pMemory[cc] != 9) ||
                                                            !bStartSpace); cc++, i++)
            {
                szValue[i] = m_pMemory[cc];
...
bool Parser::SearchKeyword_Next(int32_t &rnValue) {
    ...
    //now search for a number or end of the line.
    int i, cc;
    for (cc = m_uFilePtr, i = 0; m_pMemory[cc] != '\r' && m_pMemory[cc] != '#' &&
                                 ((m_pMemory[cc] != ' ' && m_pMemory[cc] != 9) || !bStartSpace); cc++, i++)
    {
        szValue[i] = m_pMemory[cc];
        if (szValue[i] != ' ' && szValue[i] != 9)
        { bStartSpace = true; }
