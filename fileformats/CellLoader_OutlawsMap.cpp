...
TextureHandle LoadNWX(uint8_t *pData, uint32_t uLen, const char *pszName, uint32_t &uWidth, uint32_t &uHeight) {
    ...
    for (uint32_t x = 0; x < pCell0->width; x++)
    {
        ...
        {
            //read the count.
            uint8_t count = pColumn[cidx];
            cidx++;
            if (count & 1)
            {
                uint8_t skip = (count + 1) >> 1;
                uint8_t color = pColumn[cidx];
                cidx++;
                if (color < 16) color = 0;
                //fill in color * skip
...
WorldCell *
CellLoader_OutlawsMap::Load(IDriver3D *pDriver, World *pWorld, uint8_t *pData, uint32_t uLen, const string &sFile,
    ...
    if (uLen)
    {
        ...
        //load the palette.
        TextureCache::GameFile_LoadTexture(TEXTURETYPE_PCX, 0, ARCHIVETYPE_LAB, "OUTLAWS.LAB", "HIDEOUT.PCX", false,
                                           true);
        ...
        for (int32_t t = 0; t < nTex; t++)
        {
            ...
            ahLevelTex[t] = TextureCache::GameFile_LoadTexture(TEXTURETYPE_PCX, 0, ARCHIVETYPE_LAB, "OLTEX.LAB",
                                                               szTexName, true);
        ...
        for (int32_t s = 0; s < nSectorCnt; s++)
        {
            ...
            pSector->m_FloorTexScale.Set(0.125f * 64.0f / (float) aTexWidth[(int32_t) f_fTex],
                                         0.125f * 64.0f / (float) aTexHeight[(int32_t) f_fTex]);
            pSector->m_CeilTexScale.Set(0.125f * 64.0f / (float) aTexWidth[(int32_t) f_cTex],
                                        0.125f * 64.0f / (float) aTexHeight[(int32_t) f_cTex]);
            ...
            for (int32_t w = 0; w < wallCnt; w++)
            {
                ...
                Parser::SearchKeyword_F_Next(pWall->m_texOffset[Wall::WALL_TEX_MID].x);
                pWall->m_texOffset[Wall::WALL_TEX_MID].x *= afTexScaleX[texIdx];
                Parser::SearchKeyword_F_Next(pWall->m_texOffset[Wall::WALL_TEX_MID].y);
                pWall->m_texOffset[Wall::WALL_TEX_MID].y *= afTexScaleY[texIdx];
                ...
                Parser::SearchKeyword_F_Next(pWall->m_texOffset[Wall::WALL_TEX_TOP].x);
                pWall->m_texOffset[Wall::WALL_TEX_TOP].x *= afTexScaleX[texIdx];
                Parser::SearchKeyword_F_Next(pWall->m_texOffset[Wall::WALL_TEX_TOP].y);
                pWall->m_texOffset[Wall::WALL_TEX_TOP].y *= afTexScaleY[texIdx];
                ...
                Parser::SearchKeyword_F_Next(pWall->m_texOffset[Wall::WALL_TEX_BOT].x);
                pWall->m_texOffset[Wall::WALL_TEX_BOT].x *= afTexScaleX[texIdx];
                Parser::SearchKeyword_F_Next(pWall->m_texOffset[Wall::WALL_TEX_BOT].y);
                pWall->m_texOffset[Wall::WALL_TEX_BOT].y *= afTexScaleY[texIdx];
                    ...
                    else if (nSector > -1)
                    {
                        ...
                        if (ArchiveManager::GameFile_Open(ARCHIVETYPE_LAB, "OLOBJ.LAB", szItemFile))
                        {
                            ...
                            if (uItmLen <= 2048)
                            {
                                ...
                                {
                                    if (pItmData[c] == 'A' && pItmData[c + 1] == 'N' && pItmData[c + 2] == 'I' &&
                                        pItmData[c + 3] == 'M')
                                    {
                                        ...
                                        if (toupper(spriteName[l - 3]) == 'N' && toupper(spriteName[l - 2]) == 'W' &&
                                            toupper(spriteName[l - 1]) == 'X')
                                        {
                                            ...
                                            if (ArchiveManager::GameFile_Open(ARCHIVETYPE_LAB, "OLOBJ.LAB", spriteName))
                                            {
                                                ...
                                                TextureHandle hTex = LoadNWX(pNWX_Data, uNWX_Len, spriteName, uWidth,
                                                                             uHeight);
