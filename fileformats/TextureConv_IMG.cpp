...
uint32_t TextureConv_IMG::GetHackID(const char *pszImage) {
    ...
    if (stricmp(pszImage, "COMPASS.IMG") == 0)
        uHackID = 1;
    else if (stricmp(pszImage, "PICK03I0.IMG") == 0 || stricmp(pszImage, "CHGN00I0.IMG") == 0 ||
             stricmp(pszImage, "DIE_00I0.IMG") == 0 ||
             stricmp(pszImage, "PICK02I0.IMG") == 0 || stricmp(pszImage, "PRIS00I0.IMG") == 0 ||
             stricmp(pszImage, "TITL00I0.IMG") == 0 ||
             stricmp(pszImage, "CHGN00I0.IMG") == 0)
        uHackID = 2;
    else if (stricmp(pszImage, "INFO00I0.IMG") == 0 || stricmp(pszImage, "INVE00I0.IMG") == 0 ||
             stricmp(pszImage, "INVE01I0.IMG") == 0 ||
             stricmp(pszImage, "TRAV0I00.IMG") == 0 || stricmp(pszImage, "CNFG00I0.IMG") == 0 ||
             stricmp(pszImage, "LOAD00I0.IMG") == 0 ||
             stricmp(pszImage, "AMAP00I0.IMG") == 0 || stricmp(pszImage, "MAP100I0.IMG") == 0 ||
             stricmp(pszImage, "TAMRIEL2.IMG") == 0 ||
             stricmp(pszImage, "CUST00I0.IMG") == 0 || stricmp(pszImage, "CHAR00I0.IMG") == 0 ||
             stricmp(pszImage, "CHAR01I0.IMG") == 0 ||
             stricmp(pszImage, "CHAR02I0.IMG") == 0 || stricmp(pszImage, "CHAR03I0.IMG") == 0 ||
             stricmp(pszImage, "CHAR04I0.IMG") == 0 ||
...
bool TextureConv_IMG::ConvertTexture_Pal8(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY, uint32_t &uWidth,
                                          uint32_t &uHeight, const uint8_t *pSourceData, uint32_t uLen,
                                          const uint8_t *pPalette, bool bCopyPal, uint32_t uHackID/*=0*/) {
    ImageHeader header;
...
bool TextureConv_IMG::ConvertTexture_Pal8_TexList(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY,
                                                  uint32_t &uWidth, uint32_t &uHeight, const uint8_t *pSourceData,
                                                  uint32_t uLen, const uint8_t *pPalette, int nRecord,
                                                  uint32_t uHackID) {
    int32_t index = 0;
    int16_t nRecordCount = *((int16_t *) &pSourceData[index]);
    index += 2;
    char *pszName = (char *) &pSourceData[index];
    index += 24;
    ...
    bool bCompressed = (pRec->compression == Ctx_RleCompressed || pRec->compression == Ctx_ImageRle ||
                        pRec->compression == Ctx_RecordRle);
    ...
    else if (pRec->frameCount > 0 && bCompressed == false)
    {
        ...
        int *pFrameOffsetList = (int *) &pSourceData[index];
        index += sizeof(int) * pRec->frameCount;

        //for now just grab frame 0...
        index = frameOffsStart + pFrameOffsetList[0];
        int16_t fw = *((int16_t *) &pSourceData[index]);
        index += 2;
        int16_t fh = *((int16_t *) &pSourceData[index]);
        index += 2;
        uWidth = (uint32_t) fw;
        uHeight = (uint32_t) fh;
    ...
    else
    {
        ...
        for (int h = 0; h < pRec->height; h++)
        {
            ...
            if (pRleHeaders[h].RowEncoding == 0x8000)
            {
                ...
                int16_t probe;
                uint8_t pixel;
                row_w = *((int16_t *) &pSourceData[index]);
                index += 2;
                do
                {
                    probe = *((int16_t *) &pSourceData[index]);
                    index += 2;
                    if (probe < 0)
                    {
                        probe = -probe;
                        pixel = *((uint8_t *) &pSourceData[index]);
                        index++;
                    ...
                    else if (probe > 0)
                    {
                        for (int pp = 0; pp < probe; pp++)
                        {
                            buffer[buf_idx + p] = *((uint8_t *) &pSourceData[index]);
                            index++;
                            p++;
                        }
                    }
...
bool TextureConv_IMG::ConvertTexture_8bpp(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY, uint32_t &uWidth,
                                          uint32_t &uHeight, const uint8_t *pSourceData, uint32_t uLen,
                                          uint32_t uHackID/*=0*/) {
    ImageHeader header;
    uint8_t *pImageData = NULL;
...
uint32_t TextureConv_IMG::ConvertTexture_8bpp_TexList(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY,
                                                      uint32_t &uWidth, uint32_t &uHeight, const uint8_t *pSourceData,
                                                      uint32_t uLen, int nRecord, uint32_t uHackID/*=0*/) {
    int32_t index = 0;
    int16_t nRecordCount = *((int16_t *) &pSourceData[index]);
    index += 2;
    char *pszName = (char *) &pSourceData[index];
