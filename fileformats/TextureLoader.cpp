...
bool TextureLoader::LoadTexture(uint32_t uTextureType, uint32_t uPalIndex, Archive *pTexArchive, const string &sFile,
                                bool bCopyPal) {
    ...
    if (ArchiveManager::GameFile_Open(pTexArchive, sFile.c_str()))
    {
        ...
        if (pData)
        {
            ...
            {
                m_pCurConverter->ConvertTexture_Pal8(m_pConvertedData, m_nOffsX, m_nOffsY, m_uWidth, m_uHeight, pData,
                                                     uLen, m_CurPal[uPalIndex].colors, bCopyPal, uHackID);
            }
            else if (m_uTexColorDepth == 8)
            {
                m_pCurConverter->ConvertTexture_8bpp(m_pConvertedData, m_nOffsX, m_nOffsY, m_uWidth, m_uHeight, pData,
                                                     uLen, uHackID);
            }
            else
            {
...
uint32_t
TextureLoader::LoadTexture_TexList(uint32_t uTextureType, uint32_t uPalIndex, Archive *pTexArchive, const string &sFile,
                                   int nRecord) {
    ...
    if (ArchiveManager::GameFile_Open(pTexArchive, sFile.c_str()))
    {
        ...
        if (pData)
        {
            ...
            if (m_uTexColorDepth == 32)
            {
                m_pCurConverter->ConvertTexture_Pal8_TexList(m_pConvertedData, m_nOffsX, m_nOffsY, m_uWidth, m_uHeight,
                                                             pData, uLen, m_CurPal[uPalIndex].colors, nRecord, uHackID);
            }
            else if (m_uTexColorDepth == 8)
            {
                uFrameCnt = m_pCurConverter->ConvertTexture_8bpp_TexList(m_pConvertedData, m_nOffsX, m_nOffsY, m_uWidth,
                                                                         m_uHeight, pData, uLen, nRecord, uHackID);
            }
            else
            {
                assert(0);
...
uint32_t TextureLoader::LoadTexture_NoArchive_TexList(uint32_t uTextureType, uint32_t uPalIndex, const string &sFile,
                                                      int nRecord) {
    uint32_t uFrameCnt = 1;
