...
public:
    ...
    static bool
    LoadTexture(uint32_t uTextureType, uint32_t uPalIndex, Archive *pTexArchive, const string &sFile, bool bCopyPal);

    static uint32_t
    LoadTexture_TexList(uint32_t uTextureType, uint32_t uPalIndex, Archive *pTexArchive, const string &sFile,
                        int nRecord);

    static bool LoadTexture_NoArchive(uint32_t uTextureType, uint32_t uPalIndex, const string &sFile, bool bCopyPal);

    static uint32_t
    LoadTexture_NoArchive_TexList(uint32_t uTextureType, uint32_t uPalIndex, const string &sFile, int nRecord);
    ...
    static void *GetTexExtraData(uint32_t &uDataSize) {
        uDataSize = m_uExtraDataSize;
        return m_pTexExtraData;
    }
