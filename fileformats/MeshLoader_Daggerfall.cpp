...
bool
MeshLoader_Daggerfall::Load(IDriver3D *pDriver, Mesh *pMesh, MeshCollision *pMeshCol, char *ID, int region, int type) {
    bool bSuccess = false;
    uint32_t uMeshID =
            (ID[0] - '0') * 10000 + (ID[1] - '0') * 1000 + (ID[2] - '0') * 100 + (ID[3] - '0') * 10 + (ID[4] - '0');
...
bool
MeshLoader_Daggerfall::LoadMesh(IDriver3D *pDriver, Mesh *pMesh, MeshCollision *pMeshCol, char *pData, uint32_t uLength,
                                int region, int type) {
    ...
    for (int i = 0; i < header.nPlaneCount; i++)
    {
        ...
        if (polygons[i].header.nPlanePointCount >= 3)
        {
            ...
            polygons[i].hTex = TextureCache::GameFile_LoadTexture_TexList(TEXTURETYPE_IMG, 7, ARCHIVETYPE_NONE, "",
        ...
        for (int j = 0; j < polygons[i].header.nPlanePointCount; j++)
        {
            if (j == 0)
            {
                s = polygons[i].points[j].U;
                t = polygons[i].points[j].V;
            }
            else if (j < 3)
            {
                s += polygons[i].points[j].U;
    ...
    for (int i = 0; i < header.nPlaneCount; i++)
    {
        ...
        if (polygons[i].vBounds[0].x < pMesh->m_pMaterials[m].vBounds[0].x)
            pMesh->m_pMaterials[m].vBounds[0].x = polygons[i].vBounds[0].x;
        if (polygons[i].vBounds[0].y < pMesh->m_pMaterials[m].vBounds[0].y)
            pMesh->m_pMaterials[m].vBounds[0].y = polygons[i].vBounds[0].y;
        if (polygons[i].vBounds[0].z < pMesh->m_pMaterials[m].vBounds[0].z)
            pMesh->m_pMaterials[m].vBounds[0].z = polygons[i].vBounds[0].z;

        if (polygons[i].vBounds[1].x > pMesh->m_pMaterials[m].vBounds[1].x)
            pMesh->m_pMaterials[m].vBounds[1].x = polygons[i].vBounds[1].x;
        if (polygons[i].vBounds[1].y > pMesh->m_pMaterials[m].vBounds[1].y)
            pMesh->m_pMaterials[m].vBounds[1].y = polygons[i].vBounds[1].y;
        if (polygons[i].vBounds[1].z > pMesh->m_pMaterials[m].vBounds[1].z)
            pMesh->m_pMaterials[m].vBounds[1].z = polygons[i].vBounds[1].z;
    }
