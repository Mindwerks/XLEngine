...
bool CellLoader_BloodMap::ParseHeader(char *pData, int32_t &index) {
    ...
    pMapSignature = &pData[index];
    index += 4;
    pMapVersion = (MapVersion *) &pData[index];
    index += sizeof(MapVersion);
    ...
    memcpy(&header1, &pData[index], sizeof(HeaderPart1));
    index += sizeof(HeaderPart1);
    ...
    memcpy(&header3, &pData[index], sizeof(HeaderPart3));
    index += sizeof(HeaderPart3);
...
bool CellLoader_BloodMap::ExtractSectors(char *pData, int32_t &index) {
    ...
    for (int32_t nInd = 0; nInd < m_bloodData.secCnt; nInd++)
    {
        ...
        if (m_pBloodSectors[nInd].extra > 0) // offset to extra array
        {
            ...
            m_pBloodSectors[nInd].lightFX_amp =
                    a0 < 32 ? (pExtData[13] >> 6) + a0 * 4 : (pExtData[13] >> 6) + (a0 - 32) * 4 - 128;
...
TextureHandle
CellLoader_BloodMap::AddBloodTile(int32_t picnum, uint32_t uPalIdx, uint32_t &uWidth, uint32_t &uHeight, bool bMip) {
    ...
    TextureHandle hTex = TextureCache::GameFile_LoadTexture(TEXTURETYPE_ART, uPalIdx, ARCHIVETYPE_ART, szTileArchive,
                                                            szTileName, bMip);
...
WorldCell *
CellLoader_BloodMap::Load(IDriver3D *pDriver, World *pWorld, uint8_t *pData, uint32_t uLen, const string &sFile,
    ...
    if (uLen)
    {
        ...
        for (int32_t i = 0; i < m_bloodData.secCnt; i++)
        {
            ...
            {
                pSector->m_hFloorTex = AddBloodTile(m_pBloodSectors[i].floorpicnum, m_pBloodSectors[i].floorpal, uWidth,
                                                    uHeight);
                pSector->m_FloorTexScale.Set(fFloorTexScale / (float) uWidth, fFloorTexScale / (float) uHeight);

                pSector->m_hCeilTex = AddBloodTile(m_pBloodSectors[i].ceilingpicnum, m_pBloodSectors[i].ceilingpal,
                                                   uWidth, uHeight);
            ...
            if (EngineSettings::IsServer() == false)
            {
                if (pSector->m_uFlags & Sector_2_5D::SEC_FLAGS_EXTERIOR)
                {
                    ...
                    {
                        pCell->SetSkyTex(s, AddBloodTile(m_pBloodSectors[i].ceilingpicnum + s, 0, uWidth, uHeight),
                                         uWidth, uHeight);
                    }
            ...
            if (m_pBloodSectors[i].lotag == 600)    //Z-Motion
            {
                ...
                if (m_pBloodSectors[i].floorStates[0] != m_pBloodSectors[i].floorStates[1])
                {
                    ...
                    if (m_pBloodSectors[i].startState == 0)
                    {
                        ...
                        pSector->m_pFunc->AddState(-(float) m_pBloodSectors[i].floorStates[0] / fVertScale,
                                                   LevelFunc::ST_HOLD, 0);
                        //this is the end location.
                        pSector->m_pFunc->AddState(-(float) m_pBloodSectors[i].floorStates[1] / fVertScale,
                                                   m_pBloodSectors[i].offOn_waitTime == 0 ? LevelFunc::ST_TERMINATE
                                                                                          : LevelFunc::ST_TIME,
                                                   (float) m_pBloodSectors[i].offOn_waitTime * 4.5f);
                    }
                    else    //do this or just change the initial state?
                    {
                        ...
                        //this is the end location.
                        pSector->m_pFunc->AddState(-(float) m_pBloodSectors[i].floorStates[0] / fVertScale,
                                                   m_pBloodSectors[i].onOff_waitTime == 0 ? LevelFunc::ST_TERMINATE
                                                                                          : LevelFunc::ST_TIME,
                                                   (float) m_pBloodSectors[i].onOff_waitTime * 4.5f);
                    }
                }
                else if (m_pBloodSectors[i].ceilStates[0] != m_pBloodSectors[i].ceilStates[1])
                {
                    ...
                    {
                        //this is the start location.
                        pSector->m_pFunc->AddState(-(float) m_pBloodSectors[i].ceilStates[0] / fVertScale,
                                                   LevelFunc::ST_HOLD, 0);
                        //this is the end location.
                        pSector->m_pFunc->AddState(-(float) m_pBloodSectors[i].ceilStates[1] / fVertScale,
                                                   m_pBloodSectors[i].offOn_waitTime == 0 ? LevelFunc::ST_TERMINATE
                                                                                          : LevelFunc::ST_TIME,
                                                   (float) m_pBloodSectors[i].offOn_waitTime * 4.5f);
                    }
            ...
            else if (m_pBloodSectors[i].lotag == 614)    //Sliding Marked
            {
                ...
                if (offMarker > -1 && onMarker > -1)
                {
                    ...
                    else    //do this or just change the initial state?
                    {
                        ...
                        //this is the end location.
                        pSector->m_pFunc->AddState(m, m_pBloodSectors[i].onOff_waitTime == 0 ? LevelFunc::ST_TERMINATE
                                                                                             : LevelFunc::ST_TIME,
                                                   (float) m_pBloodSectors[i].onOff_waitTime * 4.5f);
                    }
                }
            ...
            else if (m_pBloodSectors[i].lotag == 617)    //Rotate
            {
                ...
                if (pivot > -1)
                {
                    ...
                    if (m_pBloodSectors[nSec].startState == 0)
                    {
                        ...
                        //this is the end location.
                        pSector->m_pFunc->AddState(0.0f,
                                                   m_pBloodSectors[nSec].offOn_waitTime == 0 ? LevelFunc::ST_TERMINATE
                                                                                             : LevelFunc::ST_TIME,
                                                   (float) m_pBloodSectors[nSec].offOn_waitTime * 4.5f);
                    ...
                    else    //do this or just change the initial state?
                    {
                        ...
                        pSector->m_pFunc->AddState(0.0f, LevelFunc::ST_HOLD, 0);
                        //this is the end location.
                        pSector->m_pFunc->AddState(angle,
                                                   m_pBloodSectors[nSec].onOff_waitTime == 0 ? LevelFunc::ST_TERMINATE
                                                                                             : LevelFunc::ST_TIME,
                                                   (float) m_pBloodSectors[nSec].onOff_waitTime * 4.5f);
            ...
            for (w = 0; w < m_pBloodSectors[i].wallnum; w++)
            {
                ...
                Vector2 delta = pSector->m_pVertexBase[pSector->m_Walls[w].m_idx[1]] -
                                pSector->m_pVertexBase[pSector->m_Walls[w].m_idx[0]];
                ...
                if (m_pBloodWalls[ww].nextsector > -1)
                {
                    ...
                    pSector->m_Walls[w].m_mirror[0] =
                            m_pBloodWalls[ww].nextwall - m_pBloodSectors[m_pBloodWalls[ww].nextsector].wallptr;
                    ...
                    if (m_pBloodWalls[ww].cstat & 16)
                    {
                        ...
                        if (EngineSettings::IsServer() == false)
                        {
                            pSector->m_Walls[w].m_textures[Wall::WALL_TEX_MID] = AddBloodTile(
                                    m_pBloodWalls[ww].overpicnum, m_pBloodWalls[ww].pal, uWidth, uHeight);;

                            dx = (float) m_pBloodWalls[ww].xpanning / (float) uWidth;
                            dy = -2.0f * (float) m_pBloodWalls[ww].ypanning / (float) uHeight;

                            sx = (float) m_pBloodWalls[ww].xrepeat * 8.0f /
                                 (pSector->m_Walls[w].m_wallLen * (float) uWidth);
                            sy = 12.0f * (float) m_pBloodWalls[ww].yrepeat / (float) (uHeight * 8);
        ...
        for (int32_t i = 0; i < m_bloodData.spriteCnt; i++)
        {
            ...
            if ((m_pBloodSprites[i].cstat & 0x8000) == 0)
            {
                ...
                if (m_pBloodSprites[i].cstat & 0x20)
                {
                    ...
                    uint32_t uWidth, uHeight;
                    TextureHandle hTex = AddBloodTile(m_pBloodSprites[i].picnum, m_pBloodSprites[i].pal, uWidth,
                                                      uHeight, false);
                ...
                else if (m_pBloodSprites[i].cstat & 0x10)    //wall oriented sprite.
                {
                    ...
                    if (pObj)
                    {
                        ...
                        if (pSector->m_pFunc &&
                            m_pBloodSectors[m_pBloodSprites[i].sectnum].rxID == m_pBloodSprites[i].txID)
                        {
                            ...
                            {
                                pSector->m_Walls[nClosest].m_pFunc = LevelFuncMgr::CreateLevelFunc("TriggerToggle",
                                                                                                   pSector->m_uID,
                                                                                                   nClosest);
                ...
                else if ((m_pBloodSprites[i].cstat & 48) == 0)
                {
                    ...
                    if ((lotag < 1 || lotag > 19) && m_pBloodSprites[i].picnum != 0 &&
                        m_pBloodSprites[i].picnum != 3566 && m_pBloodSprites[i].xrepeat > 0 &&
                        m_pBloodSprites[i].yrepeat > 0)
                    {
                        uint32_t uWidth, uHeight;
                        TextureHandle hTex = XL_INVALID_TEXTURE;
                        ...
                        if (EngineSettings::IsServer() == false)
                        {
                            hTex = AddBloodTile(m_pBloodSprites[i].picnum, m_pBloodSprites[i].pal, uWidth, uHeight,
                                                false);
                            if (m_pBloodSprites[i].picnum == 570)
                            {
                                ...
                                {
                                    ahAnimTex[uFrameCnt] = AddBloodTile(pn, m_pBloodSprites[i].pal,
                                                                        auAnimWidth[uFrameCnt], auAnimHeight[uFrameCnt],
                                                                        false);
                                    uFrameCnt++;
                                }
                            }
                        ...
                        if (pObj)
                        {
                            ...
                            if (EngineSettings::IsServer() ==
                                false)    //There's no render component when running a server.
                            {
