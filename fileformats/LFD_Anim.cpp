...
bool LFD_Anim::LoadDELT(char *pData, int32_t len, bool bUseProperOffs) {
    ...
    Offs[0] = *((int16_t *) &pData[nIdx]);
    nIdx += 2;
    Offs[1] = *((int16_t *) &pData[nIdx]);
    nIdx += 2;
    Size[0] = *((int16_t *) &pData[nIdx]);
    nIdx += 2;
    Size[1] = *((int16_t *) &pData[nIdx]);
    nIdx += 2;
    size -= 8;
    ...
    while (size > 6)
    {
        SizeAndType = *((int16_t *) &pData[nIdx]);
        nIdx += 2;
        StartX = *((int16_t *) &pData[nIdx]);
        nIdx += 2;
        StartY = *((int16_t *) &pData[nIdx]);
        nIdx += 2;
        size -= 6;
            ...
            if (bRLE)
            {
                ...
                if (!(count & 1)) //direct
                {
                    ...
                    for (int32_t p = 0; p < count; p++)
                    {
                        ...
                        nIdx++;
                        _ImgBuffer[pIdx++] =
                                (0xff << 24) | (m_PalFile.colors[pixel].b << 16) | (m_PalFile.colors[pixel].g << 8) |
                                m_PalFile.colors[pixel].r;
                        size--;
                    }
                    num_pixels -= count;
                }
                ...
                if (bRLE)
                {
                    ...
                    if (!(count & 1)) //direct
                    {
                        ...
                        for (int p = 0; p < count; p++)
                        {
                            pixel = pImgData[nIdx];
                            nIdx++;
                            _ImgBuffer[pIdx++] = (0xff << 24) | (m_PalFile.colors[pixel].b << 16) |
                                                 (m_PalFile.colors[pixel].g << 8) | m_PalFile.colors[pixel].r;
        ...
        if (bUseProperOffs && 0)
        {
            ...
            for (int32_t y = 0, yOffs = 0; y < Size[1]; y++, yOffs += Size[0])
            {
                ...
                for (int32_t x = 0; x < Size[0]; x++)
                {
                    ...
                    {
                        _ImgBuffer[x + yOffs] =
                                (0x00 << 24) | (((int32_t) afAveClr[0]) << 16) | (((int32_t) afAveClr[1]) << 8) |
                                ((int32_t) afAveClr[2]);
                    }
                }
            }
...
void LFD_Anim::GetFrameExtents(int32_t frame, float x, float y, int32_t &frameX0, int32_t &frameY0, int32_t &frameWidth,
                               int32_t &frameHeight) {
    if (frame > m_nNumDelts - 1)
    { frame = m_nNumDelts - 1; }
...
void LFD_Anim::Render(int32_t frame, float x, float y, float maxX, float minY, float dU, float dV, bool bDistort) {
    ...
    x0 = (m_OffsX[frame] + x) * fScaleX + fOffsetX;
    x1 = m_Width[frame] * m_fScaleX * fScaleX + x0;
    y0 = (m_OffsY[frame] + y) * fScaleY + fOffsetY;
    y1 = m_Height[frame] * m_fScaleY * fScaleY + y0;
    u0 = 0.0f;
    u1 = m_u1[frame];
    v0 = 0.0f;
    v1 = m_v1[frame];
