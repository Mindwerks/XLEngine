...
public:
    ...
    ConvertTexture_Pal8(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY, uint32_t &uWidth, uint32_t &uHeight,
                        ...
                        uint32_t uHackID = 0) { return false; }

    virtual bool
    ConvertTexture_Pal8_TexList(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY, uint32_t &uWidth,
                                uint32_t &uHeight, const uint8_t *pSourceData, uint32_t uLen, const uint8_t *pPalette,
                                int nRecord, uint32_t uHackID = 0) { return false; }

    virtual bool
    ConvertTexture_8bpp(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY, uint32_t &uWidth, uint32_t &uHeight,
                        const uint8_t *pSourceData, uint32_t uLen, uint32_t uHackID = 0) { return false; }

    virtual uint32_t
    ConvertTexture_8bpp_TexList(uint8_t *pConvertedData, int32_t &nOffsX, int32_t &nOffsY, uint32_t &uWidth,
