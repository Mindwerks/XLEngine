...
bool Location_Daggerfall::Load(FILE *f, map<uint64_t, Location_Daggerfall *> &mapLoc,
                               map<string, Location_Daggerfall *> &mapNames) {
...
bool Region_Daggerfall::Load(FILE *f, map<uint64_t, Location_Daggerfall *> &mapLoc,
                             map<string, Location_Daggerfall *> &mapNames) {
    ...
    for (int r = 0; r < (int) m_uRegionCount; r++)
    {
        ...
        if (ArchiveManager::GameFile_Open(ARCHIVETYPE_BSA, "MAPS.BSA", szFileName))
        {
            ...
            if (pData)
            {
                ...
                {
                    int mapID = *((int *) &pData[index]);
                    index += 4;
                    uint8_t U1 = *((uint8_t *) &pData[index]);
                    index++;
                    uint32_t longType = *((uint32_t *) &pData[index]);
                    index += 4;
                    m_pRegions[r].m_pLocations[i].m_Lat = (*((short *) &pData[index])) & 0xFFFF;
                    index += 2;
                    m_pRegions[r].m_pLocations[i].m_Long = longType & 0xFFFF;
                    uint16_t U2 = *((uint16_t *) &pData[index]);
                    index += 2; //unknown
                    uint32_t U3 = *((uint32_t *) &pData[index]);
                    index += 4; //unknown
        ...
        if (ArchiveManager::GameFile_Open(ARCHIVETYPE_BSA, "MAPS.BSA", szFileName))
        {
            ...
            if (pData)
            {
                ...
                for (int i = 0; i < nLocationCount; i++)
                {
                    ...
                    int nPreRecCount = *((int *) &pData[index]);
                    index += 4;
                    uint8_t *PreRecords = NULL;
                    if (nPreRecCount > 0)
                    {
                        PreRecords = (uint8_t *) &pData[index];
                    ...
                    uint64_t uKey = (uint64_t) m_pRegions[r].m_pLocations[i].m_y << 32ULL |
                                    (uint64_t) m_pRegions[r].m_pLocations[i].m_x;
                    ...
                    if (pLocation->LocType == 0x00008000)
                    {
                        ...
                        int locID2 = *((int *) &pData[index]);
                        index += 4;
                        index += 4;    //unknowns
                        char BlockWidth = pData[index];
                        index++;
                        char BlockHeight = pData[index];
                        index++;
                        index += 7;    //unknowns
                        char *BlockFileIndex = &pData[index];
                        index += 64;
                        char *BlockFileNumber = &pData[index];
                        index += 64;
                        uint8_t *BlockFileChar = (uint8_t *) &pData[index];
                        index += 64;
                        m_pRegions[r].m_pLocations[i].m_BlockWidth = BlockWidth;
                        m_pRegions[r].m_pLocations[i].m_BlockHeight = BlockHeight;
                        m_pRegions[r].m_pLocations[i].m_pBlockNames = new Location_Daggerfall::LocName[BlockWidth *
                                                                                                       BlockHeight];
                        char szName[64];
                        for (int by = 0; by < BlockHeight; by++)
                        {
                            for (int bx = 0; bx < BlockWidth; bx++)
                            {
                                ...
                                if (BlockFileIndex[bidx] == 13 || BlockFileIndex[bidx] == 14)
                                {
                                    ...
                                    {
                                        sprintf(szName, "%s%s%s.RMB", _aszRMBHead[BlockFileIndex[bidx]], "GA",
                                                _aszRMBTemple[BlockFileNumber[bidx] & 0x07]);
                                    }
                                    else
                                    {
                                        sprintf(szName, "%s%s%s.RMB", _aszRMBHead[BlockFileIndex[bidx]], "AA",
                                                _aszRMBTemple[BlockFileNumber[bidx] & 0x07]);
                                    }
                                    ...
                                    {
                                        char szNameTmp[32];
                                        sprintf(szNameTmp, "%s??%s.RMB", _aszRMBHead[BlockFileIndex[bidx]],
                                                _aszRMBTemple[BlockFileNumber[bidx] & 0x07]);
                                        ArchiveManager::GameFile_SearchForFile(ARCHIVETYPE_BSA, "BLOCKS.BSA", szNameTmp,
                                                                               szName);
                                    }
                                    ArchiveManager::GameFile_Close();
                                }
                                else
                                {
                                    ...
                                    sprintf(szName, "%s%s%02d.RMB", _aszRMBHead[BlockFileIndex[bidx]], _aszRMBCharQ[Q],
                                            BlockFileNumber[bidx]);
                                    assert(Q < 12);
                                    //does this file exist?
                                    if (!ArchiveManager::GameFile_Open(ARCHIVETYPE_BSA, "BLOCKS.BSA", szName))
                                    {
                                        char szNameTmp[32];
                                        sprintf(szNameTmp, "%s??%02d.RMB", _aszRMBHead[BlockFileIndex[bidx]],
                                                BlockFileNumber[bidx]);
                                        ArchiveManager::GameFile_SearchForFile(ARCHIVETYPE_BSA, "BLOCKS.BSA", szNameTmp,
                                                                               szName);
                                    }
        ...
        if (ArchiveManager::GameFile_Open(ARCHIVETYPE_BSA, "MAPS.BSA", szFileName))
        {
            ...
            if (pData)
            {
                ...
                //pData = region data.
                int index = 0;
                uint32_t count = *((uint32_t *) &pData[index]);
                index += 4;
                ...
                DungeonOffset *pOffsets = (DungeonOffset *) &pData[index];
                index += 8 * count;
                ...
                for (int i = 0; i < (int) count; i++)
                {
                    ...
                    if (pLocation->LocType == 0)
                    {
                        ...
                        index += 8;//unknowns...
                        uint16_t blockCnt = *((uint16_t *) &pData[index]);
                        index += 2;
                        index += 5;    //unknown
                        ...
                        {
                            char X = *((uint8_t *) &pData[index]);
                            index++;
                            char Y = *((uint8_t *) &pData[index]);
                            index++;
                            uint16_t BlockNumStartIndex = *((uint16_t *) &pData[index]);
                            index += 2;
