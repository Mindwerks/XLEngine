...
void
Console::AddItem(const string &itemName, void *ptr, ConsoleItemType type, const string &itemHelp, void *pUserData) {
    ConsoleItem item;
    item.name = itemName;
    item.help = itemHelp;
    item.type = type;
...
void Console::Render() {
    ...
    {
        Vector4 dkYellow(m_aColorTable[COLOR_YELLOW].x * 0.5f, m_aColorTable[COLOR_YELLOW].y * 0.5f,
                         m_aColorTable[COLOR_YELLOW].z * 0.5f, 1.0f);
        ...
        if (iter->name == arguments[0])
        {
            switch (iter->type)
            {
                ...
                case CTYPE_VEC3:
                    ...
                    {
                        out.str("");    //clear stringstream
                        out << (*iter).name << " = " << ((Vector3 *) (*iter).varPtr)->x << " "
                            << ((Vector3 *) (*iter).varPtr)->y << " " <<
                ...
                case CTYPE_VEC4:
                    ...
                    {
                        out.str("");    //clear stringstream
                        out << (*iter).name << " = " << ((Vector4 *) (*iter).varPtr)->x << " "
                            << ((Vector4 *) (*iter).varPtr)->y << " " <<
