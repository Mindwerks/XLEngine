...
void UI_System::StartScript(const char *pszFile) {
    ...
    if (m_bScriptsLoaded == false)
    {
        ...
        ScriptSystem::RegisterFunc("void UI_CreateWindow(string &in, string &in, int, int, int, int, int, int)",
                                   asFUNCTION(UI_CreateWindow));
        ScriptSystem::RegisterFunc("void UI_CreateWindow_FromLFDFrame(string &in, int, int, int, int)",
                                   asFUNCTION(UI_CreateWindow_FromLFDFrame));
        ScriptSystem::RegisterFunc("void UI_PushWindow(string &in, string &in, int, int, int, int, int, int)",
                                   asFUNCTION(UI_PushWindow));
        ...
        ScriptSystem::RegisterFunc("void UI_RenderRect(int, int, int, int, float, float, float, float, int, int)",
                                   asFUNCTION(UI_RenderRect));
        ScriptSystem::RegisterFunc("void UI_RenderImageRect(int, int, int, int, int, float, int, int)",
                                   asFUNCTION(UI_RenderImageRect));
        ScriptSystem::RegisterFunc("void UI_SetImageUV_Range(float, float, float, float)",
                                   asFUNCTION(UI_SetImageUV_Range));
        ScriptSystem::RegisterFunc("void UI_SetImageUV_RangeI(int, int, int, int, int)",
                                   asFUNCTION(UI_SetImageUV_RangeI));
        ScriptSystem::RegisterFunc("void UI_RenderString(string &in, int, int, int, float, float, float, float)",
                                   asFUNCTION(UI_RenderString));
        ScriptSystem::RegisterFunc(
                "void UI_RenderPolygon(int, int[] &in, int[] &in, float, float, float, float, int, int)",
                asFUNCTION(UI_RenderPolygon));
        ...
        ScriptSystem::RegisterFunc("void UI_GetVirtualScreenSize(int &out, int &out)",
                                   asFUNCTION(UI_GetVirtualScreenSize));
        ...
        //LFD Anim
        ScriptSystem::RegisterFunc("int UI_CreateLFD_Anim(string &in, string &in, string &in)",
                                   asFUNCTION(UI_CreateLFD_Anim));
...
UI_Window *
UI_System::AddWindow(const string &sName, const string &sText, uint32_t uType, int32_t x, int32_t y, int32_t w,
                     int32_t h, uint32_t flags, UI_Window *parent) {
...
void UI_System::UI_CreateWindow_FromLFDFrame(string &sName, int LFDAnim_ID, int frame, int x0, int y0) {
    if (LFDAnim_ID >= 0 && LFDAnim_ID < MAX_LFD_ANIM && m_pLFD_Anim_List[LFDAnim_ID])
    {
        ...
        m_pLFD_Anim_List[LFDAnim_ID]->GetFrameExtents(frame, (float) x0 / 320.0f, (float) y0 / 200.0f, frameX0, frameY0,
...
int UI_System::UI_AddGameImage(uint32_t uImageType, string &sArchive, string &sImage) {
    ...
    if (m_Context)
    {
        UI_RenderFrame *frame = AllocRenderFrame();
        //Load Frame texture.
        frame->hFrame = TextureCache::GameFile_LoadTexture(m_auImage_TexType[uImageType], 0,
                                                           m_auImage_ArchiveType[uImageType], sArchive, sImage, false);
...
//Solid color quad.
void UI_System::UI_RenderRect(int x, int y, int w, int h, float r, float g, float b, float a, int alignHoriz,
                              int alignVert) {
    if (m_Context)
    {
        int32_t nScrWidth, nScrHeight;
...
//Solid color polygon.
void UI_System::UI_RenderPolygon(int npt, asIScriptArray *x, asIScriptArray *y, float r, float g, float b, float a,
                                 int alignHoriz, int alignVert) {
    if (m_Context)
    {
#if 0
        Vector3 polygon[16];
...
void UI_System::UI_RenderImage(int hImage, int x, int y, float intensity, int alignHoriz, int alignVert) {
    if (m_Context && hImage >= 0)
    {
        ...
        m_pDriver->SetTexture(0, frame->hFrame,
                              m_bEnableFiltering ? IDriver3D::FILTER_NORMAL_NO_MIP : IDriver3D::FILTER_POINT);
...
void
UI_System::UI_RenderImageRect(int hImage, int x, int y, int w, int h, float intensity, int alignHoriz, int alignVert) {
    if (m_Context && hImage >= 0)
    {
        UI_RenderFrame *frame = GetRenderFrame(hImage);
        int32_t nScrWidth, nScrHeight;
...
void UI_Window::RenderWindow(int x, int y, int w, int h, const string &name) {
    ...
    UI_System::UI_RenderImageRect(Editor_UI_Atlas, xx, y, 10, 41, 1.0f, UI_System::UI_Align_Left,
                                  UI_System::UI_Align_Bottom);
    xx += 10;

    UI_System::UI_SetImageUV_RangeI(Editor_UI_Atlas, 8, 56, 1, 41);
    UI_System::UI_RenderImageRect(Editor_UI_Atlas, xx, y, w - 20, 41, 1.0f, UI_System::UI_Align_Left,
                                  UI_System::UI_Align_Bottom);
    xx += (w - 20);
    ...
    UI_System::UI_RenderImageRect(Editor_UI_Atlas, xx, yy, 14, 14, 1.0f, UI_System::UI_Align_Left,
                                  UI_System::UI_Align_Bottom);
    xx += 14;

    UI_System::UI_SetImageUV_RangeI(Editor_UI_Atlas, 34, 56, 1, 14);
    UI_System::UI_RenderImageRect(Editor_UI_Atlas, xx, yy, w - 28, 14, 1.0f, UI_System::UI_Align_Left,
                                  UI_System::UI_Align_Bottom);
    xx += (w - 28);
    ...
    UI_System::UI_RenderImageRect(Editor_UI_Atlas, xx, yy, (w - 28), (h - 69), 1.0f, UI_System::UI_Align_Left,
                                  UI_System::UI_Align_Bottom);
    ...
    UI_System::UI_RenderImageRect(Editor_UI_Atlas, xx, yy, 14, (h - 69), 1.0f, UI_System::UI_Align_Left,
                                  UI_System::UI_Align_Bottom);
...
void UI_Window::RenderButton(int x, int y, int w, const string &text, int state) {
    ...
    if (state == 0)    //normal
    {
        ...
        UI_System::UI_RenderImageRect(Editor_UI_Atlas, x, y, 10, 28, 1.0f, UI_System::UI_Align_Left,
                                      UI_System::UI_Align_Bottom);

        UI_System::UI_SetImageUV_RangeI(Editor_UI_Atlas, 10, 0, 1, 28);
        UI_System::UI_RenderImageRect(Editor_UI_Atlas, x + 10, y, (w - 20), 28, 1.0f, UI_System::UI_Align_Left,
                                      UI_System::UI_Align_Bottom);

        UI_System::UI_SetImageUV_RangeI(Editor_UI_Atlas, 10, 0, 10, 28);
        UI_System::UI_RenderImageRect(Editor_UI_Atlas, x + w - 10, y, 10, 28, 1.0f, UI_System::UI_Align_Left,
                                      UI_System::UI_Align_Bottom);
