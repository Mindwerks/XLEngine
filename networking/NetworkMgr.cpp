...
uint8_t *NetworkMgr::UnpackPacket(uint8_t *packet, uint32_t packetSize, uint32_t &dataSize) {
    uint32_t uOffset = 0;
    m_PacketType = packet[0];
    uOffset++;
    m_PeerSeqNum = *((uint32_t *) &packet[1]);
...
bool NetworkMgr::CreateServer() {
    ...
    _pServer = enet_host_create(&address, EngineSettings::GetMaxPlayerCount(), uChannelCount, uIncomingBandwidth,
                                uOutgoingBandwidth);
    ...
    while (enet_host_service(_pServer, &event, 0) > 0)
    {
        switch (event.type)
        {
            case ENET_EVENT_TYPE_CONNECT:
            {
                ...
                uint8_t *packet = (uint8_t *) ScratchPad::AllocMem(uPacketSize);
                packet[0] = PTYPE_SET_MAP;
                uPacketIdx++;
                *((uint32_t *) &packet[uPacketIdx]) = m_SeqNum;
                uPacketIdx += sizeof(m_SeqNum);
            ...
            case ENET_EVENT_TYPE_RECEIVE:
                if (event.packet->dataLength > GetPacketOverhead())
                {
                    ...
                    {
                        uint32_t dataSize;
                        uint8_t *data = UnpackPacket((uint8_t *) event.packet->data, event.packet->dataLength,
                                                     dataSize);
...
void NetworkMgr::_ClientLoop() {
    ...
    while (enet_host_service(_pClient, &event, 0) > 0)
    {
        ...
        {
            case ENET_EVENT_TYPE_CONNECT:
                XL_Console::PrintF("Successful connection to the server at %x:%u.", event.peer->address.host,
...
void NetworkMgr::Client_ProcessPacket(uint8_t type, const uint8_t *data, uint32_t dataSize, uint32_t channel) {
    switch (type)
    {
        case PTYPE_SET_MAP:
        {
            ...
            const Vector3 *startPos = (const Vector3 *) &data[offset];
            offset += sizeof(Vector3);
            const int32_t *sector = (const int32_t *) &data[offset];
            XL_Console::PrintF("Setup MP parameters. Map = %s, start = (%2.2f,%2.2f,%2.2f), %d.", pszMapName,
                               startPos->x, startPos->y, startPos->z, *sector);
...
void NetworkMgr::Server_ProcessPacket(int32_t nClientID, uint8_t type, const uint8_t *data, uint32_t dataSize,
                                      uint32_t channel) {
    ScratchPad::StartFrame();
    switch (type)
    {
