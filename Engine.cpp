            ...
            while (NetworkMgr::HasRecievedStartupMsg() == false)
            {
                ...
                if (pFont)
                {
                    FontManager::BeginTextRendering();
                    {
                        ...
                        {
                            strcpy(szWaitMsg,
                                   "Error: The XL Engine timed out waiting for a server response. Aborting.");
                            xOffs = 360;
                        }
                        FontManager::RenderString((m_nWidth >> 1) - xOffs, (m_nHeight >> 1) - 12, szWaitMsg, pFont,
                                                  &Vector4::One);
                    }
...
void LoadWorldCell(uint32_t cellType, uint32_t archiveType, const char *pszArchive, const char *pszFile, int32_t worldX,
                   int32_t worldY) {
    Archive *pArchive = ArchiveManager::OpenArchive(archiveType, pszArchive);
    if (pArchive)
    {
        WorldCell *pCell = CellManager::LoadCell(g_pDriver3D, m_pEngineForAPI->GetWorld(), cellType, pArchive, pszFile,
                                                 worldX, worldY);
        if (pCell)
        {
...
void
Pathing_GetNodeVector(float &vx, float &vy, float cx, float cy, int32_t wx, int32_t wy, int32_t nodeX, int32_t nodeY) {
...
void World_Collide(void *p0, void *p1, uint32_t &uSector, float fRadius, int32_t bPassThruAdjoins) {
    m_pEngineForAPI->GetWorld()->Collide((Vector3 *) p0, (Vector3 *) p1, uSector, fRadius,
                                         bPassThruAdjoins ? true : false);
}
...
bool Engine::Loop(float fDeltaTime, bool bFullspeed) {
    ...
    if (m_pGameUpdate && XL_Console::IsActive() == false)
    {
        ...
        while (m_fTotalTime >= 1.0f)
        {
            ...
            {
                XL_Console::PrintF("^1Error: Fixed time step loop count exceeded the limit: %d. time = %2.2f",
                                   nLoopCount, m_fTotalTime);
