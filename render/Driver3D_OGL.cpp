...
void Driver3D_OGL::SetFogDensity(float fDensity) {
    if (_fFogDensity != fDensity)
    {
        glFogf(GL_FOG_DENSITY, fDensity);
        _fFogDensity = fDensity;
    }
}
...
void Driver3D_OGL::EnableStencilWriting(bool bEnable, uint32_t uValue) {
    if (bEnable)
    {
        ...
        if (_bStencilWriteEnabled == false || _uStencilValue != uValue)
        {
            glStencilFunc(GL_ALWAYS, uValue, 0xff);
            _uStencilValue = uValue;
        }
...
void Driver3D_OGL::SetTexture(int32_t slot, TextureHandle hTex, uint32_t uFilter, bool bWrap, int32_t frame) {
    ...
    {
        if (_curTex != XL_INVALID_TEXTURE)
        {
            glBindTexture(GL_TEXTURE_2D, 0);
            _curTex = XL_INVALID_TEXTURE;
        }
        if (_bTexEnabled)
        {
...
TextureHandle Driver3D_OGL::CreateTexture(uint32_t uWidth, uint32_t uHeight, uint32_t uFormat/*=TEX_FORMAT_RGBA8*/,
                                          uint8_t *pData/*=NULL*/, bool bGenMips/*=false*/, int32_t nFrameCnt/*=1*/) {
...
void Driver3D_OGL::FillTexture(TextureHandle hTex, uint8_t *pData, uint32_t uWidth, uint32_t uHeight,
                               bool bGenMips/*=false*/) {
    GLint internalFormat = GL_RGBA8;
    GLenum type = GL_UNSIGNED_BYTE;
    GLenum glFormat = GL_RGBA;
...
void Driver3D_OGL::SetVBO(uint32_t uID, uint32_t uStride, uint32_t uVBO_Flags) {
    ...
    if (_uBindBufferVB != uID)
    {
        glBindBuffer(GL_ARRAY_BUFFER, uID);
        _uBindBufferVB = uID;
        bNewBindID = true;
    }
    if (_bVtxArrayEnabled == false)
    {
        glEnableClientState(GL_VERTEX_ARRAY);
        _bVtxArrayEnabled = true;
    }
    ...
    {
        if (_bNrmlArrayEnabled == false)
        {
            glEnableClientState(GL_NORMAL_ARRAY);
            _bNrmlArrayEnabled = true;
        }
    ...
    {
        if (_bColorArrayEnabled == false)
        {
            glEnableClientState(GL_COLOR_ARRAY);
            _bColorArrayEnabled = true;
        }
    ...
    {
        if (_bTex0ArrayEnabled == false)
        {
            glEnableClientState(GL_TEXTURE_COORD_ARRAY);
            _bTex0ArrayEnabled = true;
        }
        if (_uTexCoordStride != uStride || _uTexCoordOffset != uOffset || bNewBindID)
        {
            glTexCoordPointer(2, GL_FLOAT, uStride,
                              BUFFER_OFFSET(uOffset));    //The starting point of texture coordinates
...
void Driver3D_OGL::ClearDrawData() {
    ...
    if (_bVtxArrayEnabled)
    {
        glDisableClientState(GL_VERTEX_ARRAY);
        _bVtxArrayEnabled = false;
    }
    if (_bNrmlArrayEnabled)
    {
        glDisableClientState(GL_NORMAL_ARRAY);
        _bVtxArrayEnabled = false;
    }
    if (_bColorArrayEnabled)
    {
        glDisableClientState(GL_COLOR_ARRAY);
        _bColorArrayEnabled = false;
    }
    if (_bTex0ArrayEnabled)
    {
        glDisableClientState(GL_TEXTURE_COORD_ARRAY);
        _bTex0ArrayEnabled = false;
    }
...
void Driver3D_OGL::RenderIndexedTriangles(IndexBuffer *pIB, int32_t nTriCnt, int32_t startIndex/*=0*/) {
    ...
    if (_uBindBufferIB != uIB_ID)
    {
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, uIB_ID);
        _uBindBufferIB = uIB_ID;
    }
    ...
    glDrawRangeElements(GL_TRIANGLES, 0, idxCnt, idxCnt, (uStride == 2) ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT,
                        BUFFER_OFFSET(startIndex * uStride));
}

void Driver3D_OGL::RenderScreenQuad(const Vector4 &posScale, const Vector2 &uvTop, const Vector2 &uvBot,
                                    const Vector4 &colorTop, const Vector4 &colorBot) {
    glBegin(GL_QUADS);
...
void Driver3D_OGL::RenderWorldQuad(const Vector3 &pos0, const Vector3 &pos1, const Vector2 &uv0, const Vector2 &uv1,
                                   const Vector4 &color) {
    if (_prevColor != color)
    {
        glColor4fv(&color.x);
...
void Driver3D_OGL::RenderWorldQuad(const Vector3 *posList, const Vector2 *uvList, const Vector4 &color,
                                   bool bRecieveLighting) {
    if (_prevColor != color)
    {
        glColor4fv(&color.x);
        _prevColor = color;
...
void Driver3D_OGL::RenderWorldQuad(const Vector3 *posList, const Vector2 *uvList, const Vector4 *color,
                                   bool bRecieveLighting) {
    glBegin(GL_QUADS);
    if (_prevColor != color[0])
    {
        glColor4fv(&color[0].x);
        _prevColor = color[0];
    }
    glTexCoord2f(uvList[0].x, uvList[0].y);
    glVertex3f(posList[0].x, posList[0].y, posList[0].z);
    if (_prevColor != color[1])
    {
        glColor4fv(&color[1].x);
        _prevColor = color[1];
    }
    glTexCoord2f(uvList[1].x, uvList[1].y);
    glVertex3f(posList[1].x, posList[1].y, posList[1].z);

    if (_prevColor != color[2])
    {
        glColor4fv(&color[2].x);
        _prevColor = color[2];
    }
    glTexCoord2f(uvList[2].x, uvList[2].y);
    glVertex3f(posList[2].x, posList[2].y, posList[2].z);
    if (_prevColor != color[3])
    {
        glColor4fv(&color[3].x);
        _prevColor = color[3];
    }
    glTexCoord2f(uvList[3].x, uvList[3].y);
    glVertex3f(posList[3].x, posList[3].y, posList[3].z);
