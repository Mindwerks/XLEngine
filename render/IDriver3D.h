...
public:
    LightObject(Vector3 vLoc) {
        m_vLoc = vLoc;
        m_fLightAnim0 = s_fAnimOffset0;
        s_fAnimOffset0 += 8.0f * 0.13f;
        m_fLightAnim1 = s_fAnimOffset1;
        s_fAnimOffset1 += 32.0f * 0.13f;
...
public:
    ...
    virtual void SetTexture(int32_t slot, TextureHandle hTex, uint32_t uFilter = FILTER_NORMAL, bool bWrap = true,
                            int32_t frame = -1) {};

    virtual void SetColor(Vector4 *pColor = 0) {};

    virtual TextureHandle
    CreateTexture(uint32_t uWidth, uint32_t uHeight, uint32_t uFormat = TEX_FORMAT_RGBA8, uint8_t *pData = 0,
                  bool bGenMips = false, int32_t nFrameCnt = 1) { return 0; }

    virtual void
    FillTexture(TextureHandle hTex, uint8_t *pData, uint32_t uWidth, uint32_t uHeight, bool bGenMips = false) {};
    ...
    virtual void
    RenderScreenQuad(const Vector4 &posScale, const Vector2 &uvTop, const Vector2 &uvBot, const Vector4 &colorTop,
                     const Vector4 &colorBot) {};

    virtual void RenderWorldQuad(const Vector3 &pos0, const Vector3 &pos1, const Vector2 &uv0, const Vector2 &uv1,
                                 const Vector4 &color) {};

    virtual void RenderWorldQuad(const Vector3 *posList, const Vector2 *uvList, const Vector4 &color,
                                 bool bRecieveLighting = false) {};
    ...
    //Palette
    void SetCurrentPalette(uint32_t uPalID, bool UpdatePal = false) {
        m_uPaletteID = uPalID;
        m_bUpdatePal = UpdatePal;
    }
    ...
    //Lights.
    void SetLights(int nLightCnt, const LightObject **apLights) {
        m_nLightCnt = nLightCnt;
        m_apLights = apLights;
    }
