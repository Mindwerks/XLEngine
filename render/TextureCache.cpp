...
TextureHandle TextureCache::LoadTexture(const string &szFile, bool bGenMips) {
    ...
    if (m_pLoader->Load_Image(szFile.c_str()))
    {
        ...
        TextureHandle uTexHandle = m_pDriver->CreateTexture(uWidth, uHeight, IDriver3D::TEX_FORMAT_RGBA8, pData,
                                                            bGenMips);
        //Texture definition.
        Texture tex;
...
TextureHandle
TextureCache::LoadTextureFromMem(const uint8_t *pImgBuffer, uint32_t width, uint32_t height, bool bGenMips) {
    if (pImgBuffer)
    {
        //Load the texture into the renderer.
        TextureHandle uTexHandle = m_pDriver->CreateTexture(width, height, IDriver3D::TEX_FORMAT_RGBA8,
                                                            (uint8_t *) pImgBuffer, bGenMips);
        //store previous values so we can get them easily later.
...
TextureHandle
TextureCache::LoadTextureFromMem_Pal(const uint8_t *pImgBuffer, uint32_t uPalIndex, uint32_t width, uint32_t height,
                                     const string &sName, bool bGenMips) {
    ...
    if (TextureLoader::LoadTexture_Mem(pImgBuffer, uPalIndex, width, height))
    {
        ...
        //Load the texture into the renderer.
        TextureHandle uTexHandle = m_pDriver->CreateTexture(width, height, IDriver3D::TEX_FORMAT_RGBA8, pConvertedData,
                                                            bGenMips);
...
TextureHandle
TextureCache::GameFile_LoadTexture_TexList_API(uint32_t uTextureType, uint32_t uPalIndex, uint32_t uArchiveType,
                                               const char *pszArchive, const char *pszFile, int32_t nRecord,
                                               XL_BOOL bGenMips) {
    return GameFile_LoadTexture_TexList(uTextureType, uPalIndex, uArchiveType, pszArchive, pszFile, nRecord,
                                        bGenMips != 0);
}

TextureHandle
TextureCache::GameFile_LoadTexture_TexList(uint32_t uTextureType, uint32_t uPalIndex, uint32_t uArchiveType,
                                           const string &sArchive, const string &szFile, int32_t nRecord,
                                           bool bGenMips) {
    // We have to modify the file name a little...
    char szName[32];
    ...
    if (uArchiveType != ARCHIVETYPE_NONE)    //File inside an archive format.
    {
        ...
        if (pTexArchive)
        {
            uint32_t uFrameCnt = TextureLoader::LoadTexture_TexList(uTextureType, uPalIndex, pTexArchive, szFile,
                                                                    nRecord);
            if (uFrameCnt)
            {
                ...
                //Load the texture into the renderer.
                TextureHandle uTexHandle = m_pDriver->CreateTexture(uWidth, uHeight, IDriver3D::TEX_FORMAT_RGBA8,
                                                                    pConvertedData, bGenMips, uFrameCnt);
...
TextureHandle TextureCache::GameFile_LoadTexture(uint32_t uTextureType, uint32_t uPalIndex, uint32_t uArchiveType,
                                                 const string &sArchive, const string &szFile, bool bGenMips,
                                                 bool bCopyPal) {
    // We have to modify the file name a little...
    char szName[32];
...
void TextureCache::GetTextureSize(int32_t &nOffsX, int32_t &nOffsY, uint32_t &uTexWidth, uint32_t &uTexHeight,
                                  float &fRelSizeX, float &fRelSizeY) {
