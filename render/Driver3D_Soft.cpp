        ...
        for (int cDst = 0; cDst < 256; cDst++)
        {
            ...
            else
            {
                ...
                for (int cBld = 0; cBld < 256; cBld++)
                {
                    ...
                    uint64_t rDist2 =
                            (uint64_t) (rDiff * rDiff) + (uint64_t) (gDiff * gDiff) + (uint64_t) (bDiff * bDiff);
                    if (rDist2 < uMinDist2)
                    {
                        uMinDist2 = rDist2;
                        blendIndex = cBld;
...
void Driver3D_Soft::Present() {
    ...
    {
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_FrameWidth, m_FrameHeight, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
                        m_pFrameBuffer_32bpp);
    }
    else
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, m_FrameWidth, m_FrameHeight, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
                     m_pFrameBuffer_32bpp);
    }
...
void Driver3D_Soft::Clear(bool bClearColor) {
    ...
    if (bClearColor && m_nBitDepth == 32)
    {
        ...
        for (uint32_t i = 0; i < m_FrameWidth * m_FrameHeight; i++)
        { m_pFrameBuffer_32bpp[i] = uColor; }
...
TextureHandle Driver3D_Soft::CreateTexture(uint32_t uWidth, uint32_t uHeight, uint32_t uFormat/*=TEX_FORMAT_RGBA8*/,
...
void Driver3D_Soft::FillTexture(TextureHandle hTex, uint8_t *pData, uint32_t uWidth, uint32_t uHeight,
                                bool bGenMips/*=false*/) {
...
void Driver3D_Soft::RenderIndexedTriangles(IndexBuffer *pIB, int32_t nTriCnt, int32_t startIndex/*=0*/) {
    ...
    for (int t = 0, i = startIndex; t < nTriCnt; t++, i += 3)
    {
        ...
        TriangleRasterizer::DrawClippedNGon_Indexed(this, m_pCurVBO, 3, &pIndices[i], s_uColormapID == 0 ? true : false,
                                                    alphaMode, NULL);//polyData?&polyData[t]:NULL);
    }
...
void Driver3D_Soft::RenderScreenQuad(const Vector4 &posScale, const Vector2 &uvTop, const Vector2 &uvBot,
...
void Driver3D_Soft::RenderScreenQuad_8bpp(const Vector4 &posScale, const Vector2 &uvTop, const Vector2 &uvBot,
                                          const Vector4 &colorTop, const Vector4 &colorBot) {
...
void Driver3D_Soft::RenderWorldQuad(const Vector3 &pos0, const Vector3 &pos1, const Vector2 &uv0, const Vector2 &uv1,
                                    const Vector4 &color) {
}
...
void Driver3D_Soft::RenderWorldQuad(const Vector3 *posList, const Vector2 *uvList, const Vector4 &color,
    ...
    TriangleRasterizer::DrawClippedNGon_Indexed(this, m_pCurVBO, 4, ibo.pIndices,
                                                (bRecieveLighting && s_uColormapID == 0) ? true : false, alphaMode);
}
