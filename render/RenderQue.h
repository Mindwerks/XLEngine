...
public:
    ...
    static void
    AddQuad(TextureHandle hTex, Vector3 *posList, Vector2 *uvList, const Vector4 &color, bool bForceZWrite = false);

    //Material rendering.
    static MaterialEntry *GetEntry(RenderBuckets bucket);

    //Lights.
    static void SetLightData(int nLightCnt, const LightObject **pLightList) {
        m_nCurLightCnt = nLightCnt;
        m_pCurLightList = pLightList;
    }
