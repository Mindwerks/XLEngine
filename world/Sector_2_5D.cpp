...
static float _VAdjoinBuffer[_VADJOIN_BUFFER_SIZE];
...
const float fbwidth = 1024.0f;

float fDepth[bwidth];
float fDepthPrev[bwidth];
uint16_t idBuffer[bwidth];
uint16_t sBuffer[bwidth];

//150 for Blood, 1000 for Outlaws
float Sector_2_5D::s_fFogRange = 100.0f;    //150 for Blood, make this game settable.
//136 for Blood, 136*2 for Outlaws
float s_fSkyTop = 136.0f;    // 136 for Blood - make adjustable.
//-120 for Blood, -16 Outlaws
float s_fSkyBot = -120.0f;    //-120 for Blood - make adjustable.
...
void Sector_2_5D::Render(IDriver3D *pDriver, Camera *pCamera) {
    ...
    fI = fOO255 * (float) m_uAmbientFloor;
    ...
    for (uint32_t w = 0; w < m_uWallCount; w++)
    {
        ...
        Vector3 p0, p1;
        Vector2 uv0, uv1;
        float dz = m_ZRangeCur.y - m_ZRangeCur.x;
...
bool Sector_2_5D::PointInsideSector(float x, float y) {
    Vector2 p(x, y);
    int32_t c = 0;
...
void Sector_2_5D::VisStack_Push(Vector2 &fL, Vector2 &fR, uint32_t uStartX, uint32_t uEndX, Sector_2_5D *pNext,
                                bool bUsePortalClip, Vector3 &vOffset) {
...
void Sector_2_5D::RenderSky(IDriver3D *pDriver, WorldCell *pCell) {
    ...
    float zTop = vLoc.z + s_fSkyTop;
    float zBot = vLoc.z + s_fSkyBot;
...
void Sector_2_5D::RenderSectors(IDriver3D *pDriver, WorldCell *pCell, Camera *pCamera, Sector_2_5D *pStart,
                                const vector<Sector *> &Sectors) {
    //1. Compute world space 2D edges.
    const Vector3 &vDir3D = pCamera->GetDir();
    Vector3 vLoc = pCamera->GetLoc();
    float fNearZ = 1.0f;//pCamera->GetNearZ();
    ...
    Vector2 A, B;
    float x = pCamera->GetFrustumWidth() * 10.0f;
    ...
    if (pStart->m_vAdjoin[1] != 0xffff &&
        vLoc.z > pStart->GetZ_Ceil(vLoc.x + pStart->m_vAdjOffset[1].x, vLoc.y + pStart->m_vAdjOffset[1].y, Sectors))
    {
    ...
    if (m_bUpdateVis)
    {
        ...
        do
        {
            VisStack *pVis = VisStack_Pop();
            s_vCurrentOffs = pVis->offset;
            Visibility2D(vLoc, pVis->fL, pVis->fR, pVis->uStartX, pVis->uEndX, pVis->pNext, Sectors,
                         pVis->bUsePortalClip, pDriver);
            recursion++;
        ...
        if (m_visStackCnt_VAdjoin)
        {
            ...
            for (uint32_t v = 0; v < (uint32_t) m_visStackCnt_VAdjoin; v++)
            {
                ...
                recursion = 0;
                VisStack_Clear();
                VisStack_Push(pVAdjoin->fL, pVAdjoin->fR, pVAdjoin->uStartX, pVAdjoin->uEndX, pVAdjoin->pNext, true,
                              pVAdjoin->offset);
    ...
    for (uint32_t x=0; x<bwidth; x+=uDiv)
    {
        ...
        if ( fDepth[x] != 0.0f )
        {
            ...
            fTex = 0.75f*(float)uTexID/15.0f + 0.25f;
        }

        Vector4 posScale((float)(x/uDiv), 0, (float)uDiv, 768.0f);//384.0f);
        Vector2 uv(0,0);
        float fID = 0.25f + 0.75f*(float)(idBuffer[x]&31) / 31.0f;
        float fd = sqrtf(fDepth[x]);
...
float m_fCollideHeight = 6.8f;
float m_fCollideHeight_DH = 4.5f;
float m_fStepSize = 4.5f;    //should be 3.5
float m_fJumpStepSize = 3.5f;
...
void Sector_2_5D::_AddSectorToList(int32_t s, Vector3 *p0, Vector2 &vPathMin, Vector2 &vPathMax,
                                   const vector<Sector *> &Sectors) {
    ...
    float col_z0 = pSec->GetZ_Floor(p0->x, p0->y, Sectors), col_z1;
    int32_t w;
    ...
    for (w = 0; w < pSec->m_uWallCount; w++)
    {
        ...
        if (uNextSec != 0xffff)
        {
            ...
            else
            {
                ...
                float z1 = pNext->GetZ_Ceil(p0->x, p0->y, Sectors);
...
bool _PushOutPositionList(Vector3 *p0, float fRadius, const vector<Sector *> &Sectors) {
    ...
    for (int32_t p = 0; p < 16; p++)
    {
        ...
        for (int32_t sec = 0; sec < _nCollideSecCnt; sec++)
        {
            ...
            Sector_2_5D *pSec = (Sector_2_5D *) Sectors[s];
            float col_z0 = pSec->m_ZRangeCur.x, col_z1;
            ...
            for (w = 0; w < pSec->m_uWallCount; w++)
            {
                ...
                float d2 = (v1.x - v0.x) * (v1.x - v0.x) + (v1.y - v0.y) * (v1.y - v0.y);
                float ix, iy, lu;
                if (d2 <= 0.00000001f)
                { continue; }
                ...
                if (d2 < r2)
                {
                    ...
                    if (d > 0.000001f)
                    {
                        float offs = (fRadius - d) + 0.00001f;

                        //compute wall normal...
                        Vector2 wD(v1.x - v0.x, v1.y - v0.y), wN;
                        wN.x = wD.y;
                        wN.y = -wD.x;    //normal is flipped due to winding. N = -perp. perp = (-dy,dx), so N = (dy,-dx)
                        wN.Normalize();
...
void Sector_2_5D::RayCastAndActivate(Vector3 *p0, Vector3 *p1, uint32_t &uSector, const vector<Sector *> &Sectors) {
    ...
    float ld = N.Length(), u, min_u_obj = 10.0f, min_u_sec = 10.0f;
    if (ld > 0.000001f)
    { ld = 1.0f / ld; }
    ...
    for (uint32_t w = 0; w < pSec->m_uWallCount; w++)
    {
        ...
        x0 = pSec->m_pVertexCur[i0].x;
        x1 = pSec->m_pVertexCur[i1].x;
        y0 = pSec->m_pVertexCur[i0].y;
        y1 = pSec->m_pVertexCur[i1].y;

        //find ray, line intersect.
        float denom = (y3 - y2) * (x1 - x0) - (x3 - x2) * (y1 - y0);
        float u, v;
        if (denom > 0.00001f)
        {
            denom = 1.0f / denom;
    ...
    if (nClosestWall > -1)
    {
        ...
        if (nClosestWall < 0xfffe)
        {
            ...
            if (!bSolid)
            {
                ...
                float cz0 = pNext->m_ZRangeCur.x;
                float cz1 = pNext->m_ZRangeCur.y;

                if ((pSec->m_uFlags & SEC_FLAGS_ALLOW_NONSOLID_ACTIVATE) ||
                    (pNext->m_uFlags & SEC_FLAGS_ALLOW_NONSOLID_ACTIVATE))
                {
                    if (pWallFunc == NULL)
                    {
...
void Sector_2_5D::Collide(Vector3 *p0, Vector3 *p1, uint32_t &uSector, float fRadius, const vector<Sector *> &Sectors,
                          bool bPassThruAdjoins) {
...
float Sector_2_5D::GetZ_Floor(float x, float y, const vector<Sector *> &Sectors) {
    ...
    if ((m_uFlags & SEC_FLAGS_FLOOR_SLOPE) && m_auSlopeSector[0] < 0xffff)
    {
        ...
        Vector2 D = pAnchor->m_pVertexCur[pAnchor->m_Walls[m_auSlopeAnchor[0]].m_idx[1]] -
                    pAnchor->m_pVertexCur[pAnchor->m_Walls[m_auSlopeAnchor[0]].m_idx[0]];
        Vector2 N(-D.y, D.x);
        N.Normalize();
...
float Sector_2_5D::GetZ_Ceil(float x, float y, const vector<Sector *> &Sectors) {
    ...
    if ((m_uFlags & SEC_FLAGS_CEIL_SLOPE) && m_auSlopeSector[1] < 0xffff)
    {
        ...
        Vector2 p(x - s_vCurrentOffs.x, y - s_vCurrentOffs.y);
        Vector2 D = pAnchor->m_pVertexCur[pAnchor->m_Walls[m_auSlopeAnchor[1]].m_idx[1]] -
                    pAnchor->m_pVertexCur[pAnchor->m_Walls[m_auSlopeAnchor[1]].m_idx[0]];
        Vector2 N(-D.y, D.x);
        N.Normalize();
...
void Sector_2_5D::AddObjectToRender(Object *pObj, float fIntensity, const Vector3 &vOffs) {
...
void Sector_2_5D::_DrawFloor(IDriver3D *pDriver, Sector_2_5D *pCurSec, const Vector2 *worldPos, const Vector2 &a,
                             const Vector2 &n0, const Vector2 &n1, const vector<Sector *> &Sectors) {
    const float fOO255 = (1.0f / 255.0f);
    float fI;
    Vector4 color;
    ...
    if (pCurSec->m_vAdjoin[0] == 0xffff || pCurSec->m_uFlags & Sector_2_5D::SEC_FLAGS_FLOORWATER)
    {
        ...
        {
            uvList[0].Set(posList[0].y * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[0].x * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
            uvList[1].Set(posList[1].y * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[1].x * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
            uvList[2].Set(posList[2].y * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[2].x * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
            uvList[3].Set(posList[3].y * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[3].x * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
        }
        else
        {
            uvList[0].Set(posList[0].x * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[0].y * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
            uvList[1].Set(posList[1].x * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[1].y * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
            uvList[2].Set(posList[2].x * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[2].y * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
            uvList[3].Set(posList[3].x * pCurSec->m_FloorTexScale.x + pCurSec->m_texOffset[0].x,
                          posList[3].y * pCurSec->m_FloorTexScale.y + pCurSec->m_texOffset[0].y);
        }
    ...
    if (pCurSec->m_vAdjoin[1] == 0xffff)
    {
        ...
        {
            uvList[0].Set(posList[0].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y,
                          posList[0].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x);
            uvList[1].Set(posList[1].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y,
                          posList[1].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x);
            uvList[2].Set(posList[2].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y,
                          posList[2].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x);
            uvList[3].Set(posList[3].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y,
                          posList[3].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x);
        }
        else
        {
            uvList[0].Set(posList[0].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x,
                          posList[0].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y);
            uvList[1].Set(posList[1].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x,
                          posList[1].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y);
            uvList[2].Set(posList[2].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x,
                          posList[2].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y);
            uvList[3].Set(posList[3].x * pCurSec->m_CeilTexScale.x + pCurSec->m_texOffset[1].x,
                          posList[3].y * pCurSec->m_CeilTexScale.y + pCurSec->m_texOffset[1].y);
        }
...
void Sector_2_5D::_DrawWall(IDriver3D *pDriver, Sector_2_5D *pCurSec, Sector_2_5D *pNextSec, Sector_2_5D *pBotSec,
                            Sector_2_5D *pTopSec, uint16_t w, Vector2 *worldPos, const vector<Sector *> &Sectors) {
    const float fOO255 = (1.0f / 255.0f);
    float fI;
    Vector4 color;
    ...
    Vector3 p0, p1;
    Vector2 uv0, uv1;
    float dz = pCurSec->m_ZRangeCur.y - pCurSec->m_ZRangeCur.x;
    ...
    if (pCurSec->m_Walls[w].m_flags & Wall::WALL_FLAGS_XFLIP)
    {
        uv0.x = pCurSec->m_Walls[w].m_texOffset[0].x +
                pCurSec->m_Walls[w].m_wallLen * pCurSec->m_Walls[w].m_texScale[0].x;
    ...
    float x0 = pCurSec->m_pVertexCur[pCurSec->m_Walls[w].m_idx[0]].x;
    float x1 = pCurSec->m_pVertexCur[pCurSec->m_Walls[w].m_idx[1]].x;
    float y0 = pCurSec->m_pVertexCur[pCurSec->m_Walls[w].m_idx[0]].y;
    float y1 = pCurSec->m_pVertexCur[pCurSec->m_Walls[w].m_idx[1]].y;
    ...
    float u0 = uv0.x * (1.0f - t0) + uv1.x * t0;
    ...
    if (pNextSec == NULL || (pCurSec->m_Walls[w].m_flags & Wall::WALL_FLAGS_MASKWALL))
    {
        ...
        {
            posList[0].Set(worldPos[0].x, worldPos[0].y, MAX(pCurSec->GetZ_Floor(worldPos[0].x, worldPos[0].y, Sectors),
                                                             pNextSec->GetZ_Floor(worldPos[0].x, worldPos[0].y,
                                                                                  Sectors)));
            posList[1].Set(worldPos[1].x, worldPos[1].y, MAX(pCurSec->GetZ_Floor(worldPos[1].x, worldPos[1].y, Sectors),
                                                             pNextSec->GetZ_Floor(worldPos[1].x, worldPos[1].y,
                                                                                  Sectors)));
            posList[2].Set(worldPos[1].x, worldPos[1].y, MIN(pCurSec->GetZ_Ceil(worldPos[1].x, worldPos[1].y, Sectors),
                                                             pNextSec->GetZ_Ceil(worldPos[1].x, worldPos[1].y,
                                                                                 Sectors)));
            posList[3].Set(worldPos[0].x, worldPos[0].y, MIN(pCurSec->GetZ_Ceil(worldPos[0].x, worldPos[0].y, Sectors),
                                                             pNextSec->GetZ_Ceil(worldPos[0].x, worldPos[0].y,
                                                                                 Sectors)));
        }
        ...
        {
            float fOODZ = 1.0f / dz;
            float v0 = (posList[0].z - pCurSec->m_ZRangeCur.x - s_vCurrentOffs.z) * fOODZ;
            float v1 = (posList[1].z - pCurSec->m_ZRangeCur.x - s_vCurrentOffs.z) * fOODZ;
            float v2 = (posList[2].z - pCurSec->m_ZRangeCur.x - s_vCurrentOffs.z) * fOODZ;
            float v3 = (posList[3].z - pCurSec->m_ZRangeCur.x - s_vCurrentOffs.z) * fOODZ;
    ...
    if (pNextSec)
    {
        if (pCurSec->m_Walls[w].m_flags & Wall::WALL_FLAGS_XFLIP)
        {
            uv0.x = pCurSec->m_Walls[w].m_texOffset[Wall::WALL_TEX_BOT].x +
                    pCurSec->m_Walls[w].m_wallLen * pCurSec->m_Walls[w].m_texScale[Wall::WALL_TEX_BOT].x;
        ...
        float z0 = pCurSec->GetZ_Floor(worldPos[0].x, worldPos[0].y, Sectors);
        float z1 = pCurSec->GetZ_Floor(worldPos[1].x, worldPos[1].y, Sectors);
        float z2 = pNextSec->GetZ_Floor(worldPos[0].x, worldPos[0].y, Sectors);
        float z3 = pNextSec->GetZ_Floor(worldPos[1].x, worldPos[1].y, Sectors);
...
bool ClipAgainstFrustum(const Vector2 &in_v0, const Vector2 &in_v1, Vector2 &out_v0, Vector2 &out_v1, bool &bVisible) {
    ...
    Vector2 A, B;
    A.Set(in_v0.x - _cpos.x, in_v0.y - _cpos.y);
    B.Set(in_v1.x - _cpos.x, in_v1.y - _cpos.y);
    ...
    A.Set(out_v0.x - _cpos.x, out_v0.y - _cpos.y);
    B.Set(out_v1.x - _cpos.x, out_v1.y - _cpos.y);
    ...
    float dotA = _nearZ_Nrml.Dot(A);
    float dotB = _nearZ_Nrml.Dot(B);
    if ( dotA < 0.0f && dotB < 0.0f )
    { bVisible = false; return false; }

    if ( dotA < 0.0f && dotB > 0.0f )
    {
        float s = -dotA / (dotB - dotA);
        if ( s > 0.0f && s < 1.0f )
        {
...
void Sector_2_5D::WallRasterizer(const Vector3 &cPos, uint32_t uStartX, uint32_t uEndX, Sector_2_5D *pCurSec,
                                 const vector<Sector *> &Sectors) {
    ...
    for (uint32_t w = 0; w < pCurSec->m_uWallCount; w++)
    {
        ...
        float fx0 = (ss[a].x * 0.5f + 0.5f) * (fbwidth - 1.0f);
        float fx1 = (ss[b].x * 0.5f + 0.5f) * (fbwidth - 1.0f);
        float z0 = ss[a].y;
        float z1 = ss[b].y;
        ...
        float dz = z1 - z0;
        float z0s = z0, z1s = z1;
        float s0 = ((float) x0 - fx0) / (fx1 - fx0);
        float s1 = ((float) x1 - fx0) / (fx1 - fx0);
        z0 = z0s + s0 * (z1s - z0s);
        z1 = z0s + s1 * (z1s - z0s);
        ...
        {
            float s = ((float) uStartX - (float) x0s) / ((float) x1s - (float) x0s);
            z0 = s * (z1s - z0s) + z0s;
            x0 = uStartX;
        }
        if (x1 > (int32_t) uEndX)
        {
            float s = ((float) uEndX - (float) x0s) / ((float) x1s - (float) x0s);
            z1 = s * (z1s - z0s) + z0s;
            x1 = uEndX;
        }
        float fOORange = (x1 - x0 > 0) ? 1.0f / (float) (x1 - x0) : 0.0f;
        //float dz = (z1-z0) * fOORange;
        dz = (z1 - z0) * fOORange;
        float z = z0;
    ...
    for (; x <= (int32_t) uEndX; x++)
    {
        if (idBuffer[x] != id || sBuffer[x] != sec)
        {
            ...
            if (sec == pCurSec->m_uID)
            {
                ...
                m_Camera2D.InverseTransformPointsSS_2D((float) (x0 - 0.5f) / fbwidth * 2.0f - 1.0f, z0, worldPos[0]);
                m_Camera2D.InverseTransformPointsSS_2D((float) (x1 + 0.5f) / fbwidth * 2.0f - 1.0f, z1, worldPos[1]);

                //draw the wall
                pNextSec = pCurSec->m_Walls[id].m_adjoin[0] == 0xffff ? NULL
                                                                      : (Sector_2_5D *) Sectors[pCurSec->m_Walls[id].m_adjoin[0]];
    ...
    if (sec == pCurSec->m_uID)
    {
        ...
        m_Camera2D.InverseTransformPointsSS_2D((float) (x0 - 0.5f) / fbwidth * 2.0f - 1.0f, fDepth[x0], worldPos[0]);
...
void Sector_2_5D::Visibility2D(const Vector3 &cPos, Vector2 fL, Vector2 fR, uint32_t uStartX, uint32_t uEndX,
                               Sector_2_5D *pCurSec, const vector<Sector *> &Sectors, bool bUsePortalClip,
                               IDriver3D *pDriver) {
            ...
            if (cPos.z > pCurSec->GetZ_Floor(cPos.x, cPos.y, Sectors))
            {
                ...
                if (pNextSec)
                {
                    if (m_visStackCnt_VAdjoin < _MAX_RECURSION_LEVEL)
                    {
                        ...
                        {
                            memcpy(m_visStack_VAdjoin[m_visStackCnt_VAdjoin].depth, &fDepth[uStartX],
                                   sizeof(float) * (uEndX - uStartX + 1));
    ...
    //transform all vertices into 2D projected space (x, 1/w)
    m_Camera2D.TransformPointsSS_2D(pCurSec->m_uVertexCount, pCurSec->m_pVertexCur, ssPos,
                                    Vector2(s_vCurrentOffs.x, s_vCurrentOffs.y));

    //we have to restore the depth after vertical adjoins.
    //static float fDepth_vAdjoin[4096];
    ...
    {
        float fDepthBias = 0.0f;//(ww<uAdjoinStart) ? 0.0f : 0.001f;
    ...
    for (; x <= (int32_t) uEndX; x++)
    {
        if (idBuffer[x] != id || sBuffer[x] != sec)
        {
            ...
            if (sec == pCurSec->m_uID)
            {
                ...
                m_Camera2D.InverseTransformPointsSS_2D((float) (x0 - 0.5f) / fbwidth * 2.0f - 1.0f, fDepth[x0],
                                                       worldPos[0]);
                m_Camera2D.InverseTransformPointsSS_2D((float) (x1 + 0.5f) / fbwidth * 2.0f - 1.0f, fDepth[x1],
                                                       worldPos[1]);
                ...
                if (pCurSec->m_Walls[id].m_adjoin[1] != 0xffff)
                {
                    ...
                    if (pNextSec)
                    {
                        if (m_visStackCnt_VAdjoin < _MAX_RECURSION_LEVEL)
                        {
                            ...
                            if (_uAdjoinBufferLoc < _VADJOIN_BUFFER_SIZE - (uEndX - uStartX + 2))
                            {
                                memcpy(m_visStack_VAdjoin[m_visStackCnt_VAdjoin].depth, &fDepth[x0],
                                       sizeof(float) * (x1 - x0 + 1));
