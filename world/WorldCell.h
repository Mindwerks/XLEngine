...
public:
    ...
    void GetWorldPos(int32_t &x, int32_t &y) {
        x = m_nWorldPosX;
        y = m_nWorldPosY;
    }
    ...
    void
    Collide(Vector3 *p0, Vector3 *p1, uint32_t &uSector, float fRadius, uint32_t uSectorTypeVis, bool bPassThruAdjoins,
            int32_t worldX, int32_t worldY);

    void RayCastAndActivate(Vector3 *p0, Vector3 *p1, uint32_t &uSector, uint32_t uSectorTypeVis, int32_t worldX,
                            int32_t worldY);
...
protected:
    ...
    void Collide_Recursive(Vector3 *p0, Vector3 *p1, uint32_t &uSector, float fRadius, uint32_t uSectorTypeVis,
