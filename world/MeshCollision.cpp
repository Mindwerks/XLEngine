...
void MeshCollision::Transform() {
    ...
    for (int p = 0; p < m_nPolygonCount; p++)
    {
        ...
        if (m_Polygons[p].plane.Build(m_Polygons[p].pVertexES[0], m_Polygons[p].pVertexES[1],
                                      m_Polygons[p].pVertexES[2]) == false)
        {
...
bool MeshCollision::Collide(CollisionPacket *packet, Matrix *pWorldMtx, const Vector3 &vOffset) {
    ...
    if (pWorldMtx != m_pWorldMtx || m_eRadius != packet->eRadius || vOffset.x != m_vOffset.x ||
        vOffset.y != m_vOffset.y || vOffset.z != m_vOffset.z)
    {
    ...
    for (int p = 0; p < m_nPolygonCount; p++)
    {
        ...
        {
            if (CheckTriangle(packet, m_Polygons[p].pVertexES[0], m_Polygons[p].pVertexES[ii + 1],
                              m_Polygons[p].pVertexES[ii + 2], m_Polygons[p].plane))
            {
...
bool MeshCollision::Raycast(RaycastPacket *packet, Matrix *pWorldMtx, Object *parent, Sector *pSector,
                            const Vector3 &vOffset) {
    //transform the collision mesh if needed.
    if (pWorldMtx != m_pWorldMtx || m_eRadius != Vector3::One || vOffset.x != m_vOffset.x || vOffset.y != m_vOffset.y ||
        vOffset.z != m_vOffset.z)
    {
        m_pWorldMtx = pWorldMtx;
    ...
    for (int p = 0; p < m_nPolygonCount; p++)
    {
        ...
        for (int ii = 0; ii < m_Polygons[p].numVerts - 2; ii++)
        {
            if (RayTriangle(packet, m_Polygons[p].pVertexES[0], m_Polygons[p].pVertexES[ii + 1],
                            m_Polygons[p].pVertexES[ii + 2], m_Polygons[p].plane))
            {
                bCollisionFound = true;
...
bool MeshCollision::CheckTriangle(CollisionPacket *col, const Vector3 &p1, const Vector3 &p2, const Vector3 &p3,
                                  const Plane &plane) {
    bool bCollisionFound = false;
...
bool MeshCollision::RayTriangle(RaycastPacket *col, const Vector3 &p1, const Vector3 &p2, const Vector3 &p3,
                                const Plane &plane) {
    bool bCollisionFound = false;
