...
void WorldCell::Render(IDriver3D *pDriver, Camera *pCamera, uint32_t uSectorTypeVis) {
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < (uint32_t) m_Sectors.size(); i++)
        {
            ...
            {
                if (pUpdateCam->AABBInsideFrustum(m_Sectors[i]->m_Bounds[0], m_Sectors[i]->m_Bounds[1],
                                                  m_Sectors[i]->m_x, m_Sectors[i]->m_y) == Camera::FRUSTUM_OUT)
                {
                    continue;
                }
            }
...
void WorldCell::Collide(Vector3 *p0, Vector3 *p1, uint32_t &uSector, float fRadius, uint32_t uSectorTypeVis,
                        bool bPassThruAdjoins, int32_t worldX, int32_t worldY) {
...
void WorldCell::Collide_Recursive(Vector3 *p0, Vector3 *p1, uint32_t &uSector, float fRadius, uint32_t uSectorTypeVis,
                                  bool bPassThruAdjoins, int32_t worldX, int32_t worldY) {
    ...
    bounds[0].Set(Math::Min(p0->x, p1->x) - rScaled.x, Math::Min(p0->y, p1->y) - rScaled.y,
                  Math::Min(p0->z, p1->z) - rScaled.z);
    bounds[1].Set(Math::Max(p0->x, p1->x) + rScaled.x, Math::Max(p0->y, p1->y) + rScaled.y,
                  Math::Max(p0->z, p1->z) + rScaled.z);
    ...
    else
    {
        ...
        //if ( collision.nearestDist >= veryCloseDistance )
        {
            ...
            collision.intersectionPoint =
                    collision.intersectionPoint - V * MIN(collision.nearestDist, veryCloseDistance);
        }
        ...
        if ((slidePlaneNormal.z < 0.0f && slidePlaneNormal.z > -0.98f) ||
            (slidePlaneNormal.z < 0.30f && slidePlaneNormal.z > 0.0f))
...
void WorldCell::RayCastAndActivate(Vector3 *p0, Vector3 *p1, uint32_t &uSector, uint32_t uSectorTypeVis, int32_t worldX,
                                   int32_t worldY) {
    Sector *pStartSector = m_Sectors[uSector];
    if (pStartSector->m_uTypeFlags & SECTOR_TYPE_25D)
    {
...
bool
WorldCell::Raycast(Vector3 *p0, Vector3 *p1, Vector3 *pInter, uint32_t uSectorTypeVis, int32_t worldX, int32_t worldY) {
