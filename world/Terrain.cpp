...
Terrain::Terrain(IDriver3D *pDriver, World *pWorld) {
    ...
    for (uint32_t l = 0; l < LOD_COUNT; l++)
    {
        ...
        m_LOD[l].m_pLocalHM = xlNew float[(CHUNK_TILE_WIDTH * TILE_QUAD_WIDTH + 1) *
                                          (CHUNK_TILE_WIDTH * TILE_QUAD_WIDTH + 1)];
        m_LOD[l].m_pLocalNM = xlNew Vector3[(CHUNK_TILE_WIDTH * TILE_QUAD_WIDTH + 1) *
                                            (CHUNK_TILE_WIDTH * TILE_QUAD_WIDTH + 1)];
        m_LOD[l].m_pHM_Flags = xlNew float[(CHUNK_TILE_WIDTH * TILE_QUAD_WIDTH + 1) *
                                           (CHUNK_TILE_WIDTH * TILE_QUAD_WIDTH + 1)];
    }
...
Terrain::~Terrain() {
    ...
    for (uint32_t l = 0; l < LOD_COUNT; l++)
    {
        ...
        if (m_LOD[l].m_aChunks[0].m_pChunkIB)
        {
            ...
            { xlDelete m_LOD[l].m_aChunks[i].m_pChunkIB; }
        }
...
void Terrain::LoadHeightmap() {
    ...
    if (ArchiveManager::File_Open(szFile))
    {
        ...
        int32_t index = 0;
        uint32_t OffsetSize = *((uint32_t *) &pData[index]);
        index += 4;
        uint32_t Width = *((uint32_t *) &pData[index]);
        index += 4;
        uint32_t Height = *((uint32_t *) &pData[index]);
        index += 4;
        index += 4;    //NullValue
        uint32_t DataSec1Offs = *((uint32_t *) &pData[index]);
        index += 4;
        index += 8; //unknown * 2
        uint32_t AltMapOffs = *((uint32_t *) &pData[index]);
        index += 4;
        ...
        m_hDebugHM = m_pDriver->CreateTexture(1000, 500, IDriver3D::TEX_FORMAT_FORCE_32bpp, (uint8_t *) &uHM_Data[0],
                                              false);
        //
    ...
    if (ArchiveManager::File_Open(szFile))
    {
        ...
        for (int y = 0, yOffs = 0; y < 500; y++, yOffs += 1000)
        {
            ...
            while (x < 1000)
            {
                uint16_t Count = *((uint16_t *) &pData[index]);
                index += 2;
                uint8_t Value = *((uint8_t *) &pData[index]);
                index++;
        ...
        for (uint32_t r = 0; r < WorldMap::m_uRegionCount; r++)
        {
            for (uint32_t l = 0; l < WorldMap::m_pRegions[r].m_uLocationCount; l++)
            {
                ...
                if (WorldMap::m_pRegions[r].m_pLocations[l].m_BlockWidth ||
                    WorldMap::m_pRegions[r].m_pLocations[l].m_BlockHeight)
                {
                    int32_t lumR = Math::Min((WorldMap::m_pRegions[r].m_pLocations[l].m_BlockWidth *
                                              WorldMap::m_pRegions[r].m_pLocations[l].m_BlockHeight) << 2, 255);
        ...
        m_hDebugClimate = m_pDriver->CreateTexture(1000, 500, IDriver3D::TEX_FORMAT_FORCE_32bpp,
                                                   (uint8_t *) &uClimate_Data[0], false);
        ...
        m_hPosMarker = m_pDriver->CreateTexture(3, 3, IDriver3D::TEX_FORMAT_FORCE_32bpp, (uint8_t *) &auMarkerArr[0],
                                                false);
        ...
        m_hDebugGroundTex = m_pDriver->CreateTexture(256, 256, IDriver3D::TEX_FORMAT_FORCE_32bpp,
                                                     (uint8_t *) &_groundTex[0], false);
        //
    }
}
...
void Terrain::FilterHeightMap(uint8_t *pAltMap, float *pAltMapF, float *pCoastalDist) {
    ...
    for (y = 0, yOffs = 0; y < 500; y++, yOffs += 1000)
    {
        for (x = 0; x < 1000; x++)
        {
            ...
            if (pAltMapF[yOffs + x] > max_height)
            { max_height = pAltMapF[yOffs + x]; }
        }
    }
#if 0
...
void Terrain::RenderSky(int32_t skyIndex, int32_t timeIndex, Camera *pCamera) {
    ...
    TextureHandle ahTex[] = {pSkyData->ahTexEast[timeIndex], pSkyData->ahTexWest[timeIndex],
                             pSkyData->ahTexWest[timeIndex], pSkyData->ahTexWest[timeIndex]};
...
void Terrain::RenderLOD(Camera *pCamera, int32_t lod) {
    ...
    if (m_LOD[lod].m_pVB)
    {
        ...
        if (m_pDriver->HasExtension(IDriver3D::EXT_TEXTURE_INDEX))
        {
            ...
            {
                m_pDriver->SetExtension_Data(IDriver3D::EXT_TEXTURE_INDEX, m_ahTex,
                                             m_ChunkRenderList[i]->m_TileTexArray);
...
void Terrain::RenderChunk(Camera *pCamera, int32_t lod, int32_t chunkNum) {
    if (lod > 0 || pCamera->AABBInsideFrustum(m_LOD[lod].m_aChunks[chunkNum].m_aChunkBounds.vMin,
                                              m_LOD[lod].m_aChunks[chunkNum].m_aChunkBounds.vMax, m_x, m_y) !=
                   Camera::FRUSTUM_OUT)
    {
        m_LOD[lod].m_aChunks[chunkNum].dist = m_vCamDir.Dot(
                m_LOD[lod].m_aChunks[chunkNum].m_aChunkBounds.vCen - m_vCamLoc);
...
void Terrain::BuildTerrainMeshes() {
    if (!m_pSector)
    {
        ...
        {
            TextureHandle hTex = TextureCache::GameFile_LoadTexture_TexList(TEXTURETYPE_IMG, 7, ARCHIVETYPE_NONE, "",
                                                                            szTexName, i);
            float fw, fh;
            int32_t ox, oy;
            uint32_t w, h;
    ...
    for (uint32_t l = 0; l < LOD_COUNT; l++)
    {
        ...
        m_LOD[l].m_pVB->Create(sizeof(TerrainVtx), c_numVertexEdge * c_numVertexEdge, true,
                               IDriver3D::VBO_HAS_NORMALS | IDriver3D::VBO_HAS_TEXCOORDS | IDriver3D::VBO_WORLDSPACE);
        ...
        for (int y = 0; y < CHUNK_TILE_WIDTH; y++)
        {
            for (int x = 0; x < CHUNK_TILE_WIDTH; x++)
            {
                ...
                m_LOD[l].m_aChunks[chunkIndex].m_pChunkIB->Create(TILE_QUAD_WIDTH * TILE_QUAD_WIDTH * 6,
                ...
                if (pChunkIndices)
                {
                    ...
                    for (int vy = 0; vy < TILE_QUAD_WIDTH; vy++)
                    {
                        ...
                        {
                            pChunkIndices[idxCnt++] =
                                    (vy + y * TILE_QUAD_WIDTH) * c_numVertexEdge + (vx + x * TILE_QUAD_WIDTH);
                            pChunkIndices[idxCnt++] =
                                    (vy + y * TILE_QUAD_WIDTH) * c_numVertexEdge + (vx + x * TILE_QUAD_WIDTH + 1);
                            pChunkIndices[idxCnt++] =
                                    (vy + y * TILE_QUAD_WIDTH + 1) * c_numVertexEdge + (vx + x * TILE_QUAD_WIDTH + 1);
                            ...
                            pChunkIndices[idxCnt++] =
                                    (vy + y * TILE_QUAD_WIDTH + 1) * c_numVertexEdge + (vx + x * TILE_QUAD_WIDTH + 1);
                            pChunkIndices[idxCnt++] =
                                    (vy + y * TILE_QUAD_WIDTH + 1) * c_numVertexEdge + (vx + x * TILE_QUAD_WIDTH);
                        }
                    }
                }
    ...
    {
        m_ahTex[rec] = TextureCache::GameFile_LoadTexture_TexList(TEXTURETYPE_IMG, 7, ARCHIVETYPE_NONE, "",
                                                                  "TEXTURE.002", rec);
        m_ahTex[rec + 56] = TextureCache::GameFile_LoadTexture_TexList(TEXTURETYPE_IMG, 7, ARCHIVETYPE_NONE, "",
                                                                       "TEXTURE.102", rec);
        m_ahTex[rec + 112] = TextureCache::GameFile_LoadTexture_TexList(TEXTURETYPE_IMG, 7, ARCHIVETYPE_NONE, "",
                                                                        "TEXTURE.302", rec);
        m_ahTex[rec + 168] = TextureCache::GameFile_LoadTexture_TexList(TEXTURETYPE_IMG, 7, ARCHIVETYPE_NONE, "",
                                                                        "TEXTURE.402", rec);
    }
}
...
void Terrain::BuildHeightmap(int32_t newX, int32_t newY, int32_t prevX, int32_t prevY, int32_t nRectCnt,
                             LocationRect *pRects) {
    //for now build a single LOD.
    int vIndex = 0;
                            ...
                            for (int32_t x = x0; x <= x0 + TILE_QUAD_WIDTH; x++)
                            {
                                ...
                                if (locBlend[vy + x] < 1.0f || locHeight[vy + x] == cRect->fHeight)
                                {
                                    ...
                                    if (tx >= innerRect[0] && tx < innerRect[2])
                                        locBlend[vy +
                                                 x] = yBlend;//0.0f;//locBlend[ vy + x] ? locBlend[ vy + x]*yBlend : yBlend;
                                    else
                        ...
                        for (int32_t y = y0; y <= y0 + TILE_QUAD_WIDTH; y++)
                        {
                            ...
                            for (int32_t x = x0; x <= x0 + TILE_QUAD_WIDTH; x++)
                            {
                                ...
                                if (locBlend[vy + x] < 1.0f || locHeight[vy + x] == cRect->fHeight)
                                {
                                    ...
                                    if (tx >= innerRect[0] && tx < innerRect[2])
                                        locBlend[vy +
                                                 x] = yBlend;//locBlend[ vy + x] ? locBlend[ vy + x]*yBlend : yBlend;
                                    else
                                        locBlend[vy + x] = yBlend * _xBlend[(y - y0) * 17 + (x - x0)];
                                }
                            }
            ...
            {
                {
                    ...
                    m_LOD[0].m_pLocalHM[vIndex] +=
                            ProceduralFunc::fBm(p, 3, 0.49f, 1.98f + scale2 * 0.1f) * 64.0f * scale0;
                    ...
                    {
                        m_LOD[0].m_pLocalHM[vIndex] = (1.0f - locBlend[vIndex]) * m_LOD[0].m_pLocalHM[vIndex] +
                                                      (locBlend[vIndex]) * locHeight[vIndex];
    ...
    for (int y = 0; y < CHUNK_TILE_WIDTH; y++)
    {
        for (int x = 0; x < CHUNK_TILE_WIDTH; x++)
        {
            m_LOD[0].m_aChunks[y * CHUNK_TILE_WIDTH + x].m_aChunkBounds.vMin.Set(c_startPos.x + (float) x * 1024.0f,
                                                                                 c_startPos.y + (float) y * 1024.0f,
                                                                                 1000000.0f);
            m_LOD[0].m_aChunks[y * CHUNK_TILE_WIDTH + x].m_aChunkBounds.vMax.Set(
                    c_startPos.x + (float) (x + 1) * 1024.0f, c_startPos.y + (float) (y + 1) * 1024.0f, -1000000.0f);
    ...
    for (int i = 0; i < CHUNK_COUNT; i++)
    {
        ...
        m_LOD[0].m_aChunks[i].m_aChunkBounds.vCen =
                (m_LOD[0].m_aChunks[i].m_aChunkBounds.vMin + m_LOD[0].m_aChunks[i].m_aChunkBounds.vMax) * 0.5f;
