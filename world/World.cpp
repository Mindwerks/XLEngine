...
World::World() {
    ...
    XL_Console::RegisterCmd("r_lockcam", (void *) CC_LockCamera, Console::CTYPE_FUNCTION,
                            "Lock the camera so that the visibility stops updating.", this);
    XL_Console::RegisterCmd("r_maxRecursion", &Sector::s_MaxSecDrawCnt, Console::CTYPE_UINT,
                            "Maximum portal recursion, 0 = default.", NULL);
...
void World::Collide(Vector3 *p0, Vector3 *p1, uint32_t &uSector, float fRadius, bool bPassThruAdjoins) {
    ...
    if (m_WorldCells.size() > 0)
    {
        ...
        for (; iCell != eCell; ++iCell)
        {
            ...
            pCell->Collide(p0, p1, uSector, fRadius, m_uSectorTypeVis, bPassThruAdjoins, m_pCamera->GetWorldPosX(),
                           m_pCamera->GetWorldPosY());
        }
    }

    //for now only support downward rays for terrain, fix later.
    if ((m_uSectorTypeVis & SECTOR_TYPE_EXTERIOR) && m_pTerrain && m_pTerrain->IsActive() && p0->x - p1->x == 0.0f &&
        p0->y - p1->y == 0.0f)
    {
...
void World::RayCastAndActivate(Vector3 *p0, Vector3 *p1, uint32_t &uSector) {
    ...
    if (m_WorldCells.size() > 0)
    {
        ...
        for (; iCell != eCell; ++iCell)
        {
            ...
            pCell->RayCastAndActivate(p0, p1, uSector, m_uSectorTypeVis, m_pCamera->GetWorldPosX(),
                                      m_pCamera->GetWorldPosY());
        }
    }
}
    ...
    if (m_WorldCells.size() > 0)
    {
        ...
        if (m_WorldCells.size() > 0)
        {
            ...
            for (; iCell != eCell; ++iCell)
            {
                ...
                bRayHit = pCell->Raycast(p0, p1, pInter, m_uSectorTypeVis, m_pCamera->GetWorldPosX(),
                                         m_pCamera->GetWorldPosY());
            }
        }
        ...
        //for now only support downward rays for terrain, fix later.
        if ((m_uSectorTypeVis & SECTOR_TYPE_EXTERIOR) && m_pTerrain && m_pTerrain->IsActive() &&
            p0->x - p1->x == 0.0f && p0->y - p1->y == 0.0f)
        {
            float z = m_pTerrain->GetHeight(p0->x, p0->y);
    ...
    if (m_pTerrain->IsActive())
    {
        ...
        for (int32_t wy = wy0; wy <= wy1; wy++)
        {
            for (int32_t wx = wx0; wx <= wx1; wx++)
            {
                ...
                {
                    aLocRect[nRectCnt].vRectInner.Set(pLoc->m_x, pLoc->m_y, pLoc->m_x + pLoc->m_BlockWidth,
                                                      pLoc->m_y + pLoc->m_BlockHeight);
...
void
World::GetNodeVector(Vector2 &vOffset, const Vector3 &curPos, int32_t wx, int32_t wy, int32_t nodeX, int32_t nodeY) {
    Vector3 vPos = curPos;
    vPos.x += (wx - m_pCamera->GetWorldPosX()) * 1024.0f;
    vPos.y += (wy - m_pCamera->GetWorldPosY()) * 1024.0f;

    vOffset.Set(m_PathNodes[nodeY * 48 + nodeX].localWorldPos.x - vPos.x,
                m_PathNodes[nodeY * 48 + nodeX].localWorldPos.y - vPos.y);
}
...
bool World::GetRandomNode(int32_t &nodeX, int32_t &nodeY, Vector3 &outPos, int32_t &outWorldX, int32_t &outWorldY) {
    ...
    while (!bFound && nIter < 32)
    {
        ...
        if (m_PathNodes[nodeY * 48 + nodeX].bOpen)
        {
            ...
            if (m_PathNodes[(nodeY - 1) * 48 + nodeX].bOpen || m_PathNodes[(nodeY - 1) * 48 + nodeX - 1].bOpen ||
                m_PathNodes[(nodeY - 1) * 48 + nodeX + 1].bOpen ||
                m_PathNodes[(nodeY + 1) * 48 + nodeX].bOpen || m_PathNodes[(nodeY + 1) * 48 + nodeX - 1].bOpen ||
                m_PathNodes[(nodeY + 1) * 48 + nodeX + 1].bOpen ||
                m_PathNodes[nodeY * 48 + nodeX - 1].bOpen || m_PathNodes[nodeY * 48 + nodeX + 1].bOpen)
            {
                ...
                outPos.z = m_pTerrain->GetHeight(m_PathNodes[nodeY * 48 + nodeX].localWorldPos.x,
                                                 m_PathNodes[nodeY * 48 + nodeX].localWorldPos.y);
