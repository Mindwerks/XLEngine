...
public:
    ...
    void AllocGameData(uint32_t uSize) {
        m_pDataComp = xlMalloc(uSize);
        m_bAllocGameData = true;
    }
    ...
    void Render(IDriver3D *pDriver, float fIntensity, const Vector3 &vOffset) {
        if (m_pRenderComp && (m_uFlags & OBJFLAGS_ACTIVE))m_pRenderComp->Render(this, pDriver, fIntensity, vOffset);
    }

    void SetCollisionComponent(CollisionComponent *pCollision) { m_pCollisionComp = pCollision; }

    bool Collide(CollisionPacket *packet, const Vector3 &vOffset) {
        if (m_pCollisionComp && m_bCollisionEnable && (m_uFlags & OBJFLAGS_ACTIVE))
            return m_pCollisionComp->Collide(packet, &m_worldMtx, vOffset);
        else return false;
    }

    bool Raycast(RaycastPacket *packet, Sector *pSector, const Vector3 &vOffset) {
        if (m_pCollisionComp && (m_uFlags & OBJFLAGS_ACTIVE))
            return m_pCollisionComp->Raycast(packet, &m_worldMtx, this, pSector, vOffset);
        else return false;
    }
    ...
    void GetWorldBounds(Vector3 &vMin, Vector3 &vMax) {
        vMin = m_worldBounds[0];
        vMax = m_worldBounds[1];
    }
    ...
    void SetWorldBounds(const Vector3 &vMin, const Vector3 &vMax) {
        m_worldBounds[0] = vMin;
        m_worldBounds[1] = vMax;
    }

    float GetBoundingSphere(Vector3 &vCen) {
        vCen = m_worldCen;
        return m_fRadius;
    }

    void SetBoundingSphere(const Vector3 &vCen, float fRadius) {
        m_worldCen = vCen;
        m_fRadius = fRadius;
    }
    ...
    void AddLight(const LightObject *pLight) {
        if (m_nLightCount < MAX_LIGHT_COUNT)
            m_apLights[m_nLightCount++] = pLight;
    }

    const LightObject **GetLightList(int &rnLightCnt) {
        rnLightCnt = m_nLightCount;
        return m_apLights;
    }
    ...
    void SetWorldPos(int32_t x, int32_t y) {
        m_ObjPhysicsData.m_worldX = x;
        m_ObjPhysicsData.m_worldY = y;
    }
